  ...
  = describe "FreeC.Backend.Coq.Converter.TypeDecl.convertTypeSynDecl" $ do
    ...
    it "expands type synonyms in mutually recursive data type declarations"
      ...
      $ do
        ...
        shouldConvertLocalTypeDeclsTo
          ...
          ++ "Fixpoint shareArgsTree_ {Shape : Type} {Pos : Shape -> Type} "
          ...
          ++ ":= let fix shareArgsListTree_ {a0 : Type} "
          ...
          ++ "cbneed Shape Pos shareArgsListTree_ fx2 >>= (fun sx2 => "
          ++ "pure (cons sx1 sx2))) "
          ++ "end "
          ++ "in match x with "
          ...
          ++ "pure (leaf sx)) "
          ++ "| branch fx0 => "
          ++ "cbneed Shape Pos shareArgsListTree_ fx0 >>= (fun sx0 => "
          ++ "pure (branch sx0)) "
          ++ "end. "
          ++ "Instance ShareableArgsTree_ {Shape : Type} "
          ++ "{Pos : Shape -> Type} {a : Type} "
          ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
    ...
    it "sorts type synonym declarations topologically" $ shouldSucceedWith $ do
      ...
      shouldConvertLocalTypeDeclsTo
        ...
        ++ "Fixpoint shareArgsFoo {Shape : Type} {Pos : Shape -> Type} "
        ++ "`{Injectable Share.Shape Share.Pos Shape Pos} (x : Foo Shape Pos) "
        ++ ": Free Shape Pos (Foo Shape Pos) := let 'foo fx fx0 := x "
        ++ "in cbneed Shape Pos shareArgsFoo fx >>= (fun sx => "
        ++ "cbneed Shape Pos shareArgsFoo fx0 >>= (fun sx0 => "
        ++ "pure (foo sx sx0))). "
        ++ "Instance ShareableArgsFoo {Shape : Type} {Pos : Shape -> Type} "
        ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
    ...
    context "Translation of types and type synonyms" $ do
      it "translates non-polymorphic data types correctly"
        ...
        $ do
          ...
          shouldConvertLocalTypeDeclsTo (NonRecursive "data Foo = Bar | Baz")
            ...
            ++ "Fixpoint shareArgsFoo {Shape : Type} {Pos : Shape -> Type} "
            ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
            ++ "(x : Foo Shape Pos) : Free Shape Pos (Foo Shape Pos) "
            ++ ":= match x with "
            ++ "| bar => pure bar "
            ++ "| baz => pure baz "
            ++ "end. "
            ++ "Instance ShareableArgsFoo {Shape : Type} "
            ++ "{Pos : Shape -> Type} "
            ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
            ++ ": ShareableArgs Shape Pos (Foo Shape Pos) "
            ++ ":= { shareArgs := shareArgsFoo }. "
      it "translates polymorphic data types correctly" $ shouldSucceedWith $ do
        ...
        shouldConvertLocalTypeDeclsTo
          ...
          ++ ":= { nf' := nf'Foo__ }. "
          ++ "(* ShareableArgs instance for Foo *) "
          ++ "Fixpoint shareArgsFoo__ {Shape : Type} "
          ++ "{Pos : Shape -> Type} {a a0 : Type} "
          ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
          ++ "`{ShareableArgs Shape Pos a} `{ShareableArgs Shape Pos a0} "
          ++ "(x : Foo Shape Pos a a0) : Free Shape Pos (Foo Shape Pos a a0) "
          ++ ":= match x with "
          ...
          ++ "pure (baz sx0)) "
          ++ "end. "
          ++ "Instance ShareableArgsFoo__ {Shape : Type} "
          ++ "{Pos : Shape -> Type} {a a0 : Type} "
          ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
          ++ "`{ShareableArgs Shape Pos a} `{ShareableArgs Shape Pos a0} "
      ...
      it "renames type variables with same name as generated constructors"
        ...
        $ do
          ...
          shouldConvertLocalTypeDeclsTo (NonRecursive "data Foo a = A a")
            ...
            ++ ":= { nf' := nf'Foo_ }. "
            ++ "(* ShareableArgs instance for Foo *) "
            ++ "Fixpoint shareArgsFoo_ {Shape : Type} {Pos : Shape -> Type} "
            ++ "{a0 : Type} `{Injectable Share.Shape Share.Pos Shape Pos} "
            ++ "`{ShareableArgs Shape Pos a0} (x : Foo Shape Pos a0) "
            ++ ": Free Shape Pos (Foo Shape Pos a0) := let 'a fx := x in "
            ++ "cbneed Shape Pos shareArgs fx >>= (fun sx => pure (a sx)). "
            ++ "Instance ShareableArgsFoo_ {Shape : Type} "
            ++ "{Pos : Shape -> Type} {a0 : Type} "
            ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
            ++ "`{ShareableArgs Shape Pos a0} "
      ...
      it "translates mutually recursive data types correctly"
        ...
        $ do
          ...
          shouldConvertLocalTypeDeclsTo
            ...
            ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
            ++ "(x : Foo Shape Pos) "
            ++ ": Free Shape Pos (Foo Shape Pos) := let 'foo fx := x in "
            ++ "cbneed Shape Pos shareArgsBar fx >>= (fun sx => "
            ++ "pure (foo sx)) with "
            ++ "shareArgsBar {Shape : Type} {Pos : Shape -> Type} "
            ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
            ++ "(x : Bar Shape Pos) : Free Shape Pos (Bar Shape Pos) "
            ++ ":= let 'bar fx := x in "
            ++ "cbneed Shape Pos shareArgsFoo fx >>= (fun sx => "
            ++ "pure (bar sx)). "
            ++ "Instance ShareableArgsFoo {Shape : Type} "
            ++ "{Pos : Shape -> Type} "
            ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
            ++ ": ShareableArgs Shape Pos (Foo Shape Pos) "
            ++ ":= { shareArgs := shareArgsFoo }. "
            ++ "Instance ShareableArgsBar {Shape : Type} "
            ++ "{Pos : Shape -> Type} "
            ++ "`{Injectable Share.Shape Share.Pos Shape Pos} "
    ...
    context "Generation of induction schemes" $ do
      it "creates a correct induction scheme" $ shouldSucceedWith $ do
        ...
        shouldConvertLocalTypeDeclsTo
          ...
          ++ ": Free Shape Pos (Foo Shape Pos a) := let 'foo fx fx0 fx1 := x "
