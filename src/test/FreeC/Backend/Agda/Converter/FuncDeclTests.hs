...
testConvertFuncDecls = describe
  "FreeC.Backend.Agda.Converter.FuncDecl.convertFuncDecl"
  $ do
    ...
    it "translates if-then-else correctly"
      $ shouldSucceedWith
      $ do
        "Integer" <- defineTestTypeCon "Integer" 0 []
        ...
        shouldConvertFuncDeclsTo
          (NonRecursive
           $ "even (n :: Integer) :: Boolean " ++ "= if n then True else False")
