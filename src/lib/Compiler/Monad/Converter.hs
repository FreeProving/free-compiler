...
-- | Runs the given converter and returns its result.
--
--   Like 'localEnv', the modifications to the environment are discarded.
--   Unlike 'localEnv', the environment remains at top-level and all
--   top-level entries are kept.
--
--   This function effectively isolates the environment from (fresh/type)
--   variables generated by the converter and temporary modification to
--   the type signatures of functions.
sectionEnv :: Monad m => ConverterT m a -> ConverterT m a
sectionEnv converter = do
  env       <- getEnv
  (x, env') <- localEnv (liftM2 (,) converter getEnv)
  let addedEntries = topLevelEntries env' `Set.difference` topLevelEntries env
  mapM_ addSectionEntry (Set.toList addedEntries)
  return x
 where
  -- | Gets all top-level entries declared within the given environment.
  topLevelEntries :: Environment -> Set EnvEntry
  topLevelEntries env =
    Set.filter isTopLevelEntry $ Set.unions $ map fst $ Map.elems
      (envEntries env)

  addSectionEntry :: Monad m => EnvEntry -> ConverterT m ()
  addSectionEntry entry = do
    -- Generate the qualified and unqualified Haskell name.
    let qualName@(HS.Qual _ name) = entryName entry
        unqualName                = HS.UnQual name
    -- Associate the entry with both its qualified and unqualified name.
