...
If we instantiate the functor $f$ with a data type \haskell{Zero} that does not have any constructor, we cannot create a value of type \haskell{Free Zero a} using the \haskell{Impure} constructor (in a total setting).
...
\end{minted}
Similarly, a data type \haskell{One}, with a single inhabitant, can be used to model the \haskell{Maybe} monad.
\begin{minted}{haskell}
  data One a = One
  data Maybe a = Just a | Nothing
\end{minted}
The \haskell{Pure} constructor corresponds to \haskell{Just} and \haskell{Impure One} corresponds to \haskell{Nothing}.
