...
shouldParse input expectedOutput
  = (parseTestIR input :: Parseable a => Reporter a)
  `shouldReturn` expectedOutput
...
testIRParser :: Spec
testIRParser = describe "FreeC.Frontend.IR.Parser"
  $ do
    testNameParser
    testQNameParser
    testTypeParser
    testTypeSchemeParser
...
testNameParser :: Spec
testNameParser = context "names"
  $ do
    it "accepts variable identifiers"
      $ do
    ...
    it "accepts constructor identifiers"
      $ do
        "X" `shouldParse` IR.Ident "X"
        "Foo" `shouldParse` IR.Ident "Foo"
        "Bar'" `shouldParse` IR.Ident "Bar'"
        ...
        "Fizz_Buzz" `shouldParse` IR.Ident "Fizz_Buzz"
        "Qux2" `shouldParse` IR.Ident "Qux2"
    it "accepts variable symbols"
      $ do
        "(>>=)" `shouldParse` IR.Symbol ">>="
        "(,)" `shouldParse` IR.Symbol ","
        "(++)" `shouldParse` IR.Symbol "++"
    it "accepts constructor symbols"
      $ do
        "()" `shouldParse` IR.Symbol ""
        "(:)" `shouldParse` IR.Symbol ":"
        "(:|)" `shouldParse` IR.Symbol ":|"
        "(:.:)" `shouldParse` IR.Symbol ":.:"
    it "rejects identifiers starting with an apostrophe"
      $ do
...
testQNameParser = context "qualifiable names"
  $ do
    it "accepts unqualified names"
      $ do
...
testTypeParser :: Spec
testTypeParser = context "type expressions"
  $ do
    ...
    it "accepts unqualified type variables"
      $ do
        "a" `shouldParse` a
    it "rejects qualified type variables"
      $ do
        shouldBeParseError (parseTestType "M.a")
    it "accepts type constructors"
      $ do
        "A" `shouldParse` a'
    it "accepts function types"
      $ do
        "a -> b" `shouldParse` IR.FuncType NoSrcSpan a b
    it "parses function types right associative"
      $ do
        "a -> b -> c"
          `shouldParse` IR.FuncType NoSrcSpan a (IR.FuncType NoSrcSpan b c)
    it "accepts type application"
      $ do
        "A b" `shouldParse` IR.TypeApp NoSrcSpan a' b
    it "parses type application left associative"
      $ do
        "A b c"
          `shouldParse` IR.TypeApp NoSrcSpan (IR.TypeApp NoSrcSpan a' b) c
    it "accepts types with parentheses"
      $ do
        "(a -> b) -> c"
          `shouldParse` IR.FuncType NoSrcSpan (IR.FuncType NoSrcSpan a b) c
        "A (B c)"
          `shouldParse` IR.TypeApp NoSrcSpan a' (IR.TypeApp NoSrcSpan b' c)

-- | Test group for 'Parseable' instance of 'IR.TypeScheme'.
testTypeSchemeParser :: Spec
testTypeSchemeParser = context "type schemes"
  $ do
    let a = IR.TypeVarDecl NoSrcSpan "a"
        b = IR.TypeVarDecl NoSrcSpan "b"
        t = IR.FuncType NoSrcSpan (IR.TypeVar NoSrcSpan "a")
          (IR.TypeVar NoSrcSpan "b")
    it "accepts type schemes without explicit forall"
      $ do
        "a -> b" `shouldParse` IR.TypeScheme NoSrcSpan [] t
    it "accepts type schemes with explicit empty forall"
      $ do
        "forall. a -> b" `shouldParse` IR.TypeScheme NoSrcSpan [] t
    it "accepts type schemes with explicit non-empty forall"
      $ do
...
testSynTypeDeclParser = context "type synonym declarations"
  $ do
    it "accepts type synonym declarations without type arguments"
      $ "type Foo = Bar"
      `shouldParse` IR.TypeSynDecl NoSrcSpan
...
testDataDeclParser = context "data type declarations"
  $ do
    ...
    it "accepts data type declarations whose constructors have fields"
      $ do
        let a  = IR.TypeVarDecl NoSrcSpan "a"
            b  = IR.TypeVarDecl NoSrcSpan "b"
    ...
    it "accepts data type declarations with qualified names"
      $ do
        let foo' = IR.DeclIdent NoSrcSpan (IR.Qual "M" (IR.Ident "Foo"))
        shouldParse "data M.Foo" $ IR.DataDecl NoSrcSpan foo' [] []
    it "accepts data type declarations with qualified constructor names"
      $ do
...
testTypeSigParser = context "type signatures"
  $ do
    ...
    it "accepts type signatures without forall"
      $ do
        "f :: a -> b" `shouldParse` IR.TypeSig NoSrcSpan [f] t
    it "accepts type signatures with forall"
      $ do
        "f :: forall a b. a -> b" `shouldParse` IR.TypeSig NoSrcSpan [f] t'
    it "accepts type signatures for multiple functions"
      $ do
        "f, g :: a -> b" `shouldParse` IR.TypeSig NoSrcSpan [f, g] t
    it "accepts type signatures for qualified names"
      $ do
...
testExprParser :: Spec
testExprParser = context "expressions"
  $ do
    testExprTypeParser
    testConExprParser
    testVarExprParser
...
testExprTypeParser = context "type annotated expressions"
  $ do
    it "does not require parentheses around type annotated expressions"
      $ do
        "x :: a"
          `shouldParse` IR.Var NoSrcSpan (IR.UnQual (IR.Ident "x"))
          (Just (IR.TypeScheme NoSrcSpan [] (IR.TypeVar NoSrcSpan "a")))
    it "allows parentheses around type annotated expressions"
      $ do
        "(x :: a)"
          `shouldParse` IR.Var NoSrcSpan (IR.UnQual (IR.Ident "x"))
          (Just (IR.TypeScheme NoSrcSpan [] (IR.TypeVar NoSrcSpan "a")))
    it "prefers the outermost type annotation"
      $ do
        "(x :: a) :: b"
          `shouldParse` IR.Var NoSrcSpan (IR.UnQual (IR.Ident "x"))
          (Just (IR.TypeScheme NoSrcSpan [] (IR.TypeVar NoSrcSpan "b")))
    it "rejects multiple type annotations without parentheses"
      $ do
        shouldBeParseError (parseTestExpr "x :: a :: b")

-- | Test group for 'Parseable' instance of constructor expressions.
testConExprParser :: Spec
testConExprParser = context "constructors"
  $ do
    it "accepts constructors"
      $ do
        "Foo"
          `shouldParse` IR.Con NoSrcSpan (IR.UnQual (IR.Ident "Foo")) Nothing
    it "accepts constructors with symbolic names"
      $ do
        ...
        "(,)"
          `shouldParse` IR.Con NoSrcSpan (IR.UnQual (IR.Symbol ",")) Nothing
    it "accepts constructors with qualified names"
      $ do
        "M.Foo"
...
testAppExprParser = context "function applications"
  $ do
    ...
    it "accepts function applications"
      $ do
        "f x" `shouldParse` fx
    it "parses function applications left associative"
      $ do
        "f x y" `shouldParse` IR.App NoSrcSpan fx y Nothing
    it "accepts parentheses in function applications"
      $ do
        "f (g x)" `shouldParse` IR.App NoSrcSpan f gx Nothing
    it "accepts function applications with type annotations"
      $ do
        "f x :: a" `shouldParse` IR.App NoSrcSpan f x (Just a)
    it "accepts function applications with type annotations for argument"
      $ do
        "f (x :: a)" `shouldParse` IR.App NoSrcSpan f x' Nothing
    it "accepts function applications with type annotations for callee"
      $ do
...
testLambdaExprParser = context "lambda abstractions"
  $ do
    let a     = IR.TypeScheme NoSrcSpan [] a'
        a'    = IR.TypeVar NoSrcSpan "a"
        ...
        x     = IR.Var NoSrcSpan (IR.UnQual (IR.Ident "x")) Nothing
        x'    = IR.Var NoSrcSpan (IR.UnQual (IR.Ident "x")) (Just a)
    it "accepts lambda abstractions with a single argument"
      $ do
        "\\x -> x" `shouldParse` IR.Lambda NoSrcSpan [xPat] x Nothing
    it "accepts lambda abstractions with multiple arguments"
      $ do
        "\\x y -> x" `shouldParse` IR.Lambda NoSrcSpan [xPat, yPat] x Nothing
    it "accepts nested lambda abstractions"
      $ do
        "\\x -> \\y -> x"
          `shouldParse` IR.Lambda NoSrcSpan [xPat]
          (IR.Lambda NoSrcSpan [yPat] x Nothing) Nothing
    it "accepts lambda abstractions with type annotated arguments"
      $ do
...
testIfExprParser = context "if expressions"
  $ do
    ...
    it "accepts if expressions with type annotated conditions"
      $ do
        "if x :: a then y else z" `shouldParse` IR.If NoSrcSpan x' y z Nothing
    it "accepts if expressions with type annotated conditions in parentheses"
      $ do
        "if (x :: a) then y else z"
          `shouldParse` IR.If NoSrcSpan x' y z Nothing
    it "accepts if expressions with type annotated branches"
      $ do
        "if x then y :: a else z :: a"
          `shouldParse` IR.If NoSrcSpan x y' z' Nothing
    it "accepts if expressions with type annotated branches in parentheses"
      $ do
        "if x then (y :: a) else (z :: a)"
          `shouldParse` IR.If NoSrcSpan x y' z' Nothing
    it "accepts if expressions with type annotations"
      $ do
        "(if x then y else z) :: a"
          `shouldParse` IR.If NoSrcSpan x y z (Just a)

-- | Test group for 'Parseable' instance of @case@ expressions.
testCaseExprParser :: Spec
testCaseExprParser = context "case expressions"
  $ do
    let a           = IR.TypeScheme NoSrcSpan [] a'
        a'          = IR.TypeVar NoSrcSpan "a"
    ...
    it "accepts empty case expressions"
      $ do
        "case s of {}" `shouldParse` IR.Case NoSrcSpan s [] Nothing
    it "accepts case expressions with a single alternative"
      $ do
        "case s of { Foo -> x }"
          `shouldParse` IR.Case NoSrcSpan s [IR.Alt NoSrcSpan fooPat [] x]
          Nothing
    it "accepts case expressions with multiple alternatives"
      $ do
        "case s of { Foo -> x; Bar -> y }"
          `shouldParse` IR.Case NoSrcSpan s
          [IR.Alt NoSrcSpan fooPat [] x, IR.Alt NoSrcSpan barPat [] y] Nothing
    it "accepts case expressions with trailing semicolons"
      $ do
        "case s of { Foo -> x; Bar -> y; }"
          `shouldParse` IR.Case NoSrcSpan s
          [IR.Alt NoSrcSpan fooPat [] x, IR.Alt NoSrcSpan barPat [] y] Nothing
    it "accepts case expressions with variable patterns"
      $ do
        "case s of { Foo x y -> x }"
          `shouldParse` IR.Case NoSrcSpan s
          [IR.Alt NoSrcSpan fooPat [xPat, yPat] x] Nothing
    it "accepts case expressions with strict variable patterns"
      $ do
        "case s of { Foo !x !y -> x }"
          `shouldParse` IR.Case NoSrcSpan s
          [IR.Alt NoSrcSpan fooPat [xPatStrict, yPatStrict] x] Nothing
    it "accepts case expressions with type annotated variable patterns"
      $ do
        "case s of { Foo (x :: a) -> x }"
    ...
    it "accepts case expressions with type annotations"
      $ do
        "case s of { Foo x -> x } :: a"
          `shouldParse` IR.Case NoSrcSpan s [IR.Alt NoSrcSpan fooPat [xPat] x]
          (Just a)
    it "accepts case expressions with type annotated scrutinees"
      $ do
        "case s :: a of { Foo x -> x }"
          `shouldParse` IR.Case NoSrcSpan s' [IR.Alt NoSrcSpan fooPat [xPat] x]
          Nothing
...
testErrorTermParser = context "error terms"
  $ do
    let a = IR.TypeScheme NoSrcSpan [] (IR.TypeVar NoSrcSpan "a")
    it "accepts 'undefined'"
      $ do
        "undefined" `shouldParse` IR.Undefined NoSrcSpan Nothing
    it "accepts 'undefined' with type annotation"
      $ do
        "undefined :: a" `shouldParse` IR.Undefined NoSrcSpan (Just a)
    it "accepts 'error'"
      $ do
        "error \"...\"" `shouldParse` IR.ErrorExpr NoSrcSpan "..." Nothing
    it "accepts 'error' with type annotation"
      $ do
        "error \"...\" :: a"
          `shouldParse` IR.ErrorExpr NoSrcSpan "..." (Just a)
    it "rejects unapplied 'error'"
      $ do
        shouldBeParseError (parseTestExpr "error")
    it "rejects standalone string literal"
      $ do
        shouldBeParseError (parseTestExpr "\"...\"")
    it "requires parentheses around 'error' in application"
      $ do
...
testTypeAppExprParser = context "visible type applications"
  $ do
    let a = IR.TypeVar NoSrcSpan "a"
        f = IR.Var NoSrcSpan (IR.UnQual (IR.Ident "f")) Nothing
    it "accepts visible type application of functions"
      $ do
        "f @a" `shouldParse` IR.TypeAppExpr NoSrcSpan f a Nothing
    it "accepts visible type application of constructors"
      $ do
        "C @a" `shouldParse` IR.TypeAppExpr NoSrcSpan f a Nothing
    it "accepts visible type application of 'undefined'"
      $ do
        "undefined @a"
          `shouldParse` IR.TypeAppExpr NoSrcSpan
          (IR.Undefined NoSrcSpan Nothing) a Nothing
    it "accepts visible type application of 'error'"
      $ do
        "error @a \"...\""
          `shouldParse` IR.TypeAppExpr NoSrcSpan
          (IR.ErrorExpr NoSrcSpan "..." Nothing) a Nothing
    it "requires parentheses around visible type application in application"
      $ do
        shouldBeParseError (parseTestExpr "f g @a")
    it "rejects visible type application of literals"
      $ do
        shouldBeParseError (parseTestExpr "42 @a")
    it "rejects visible type application of parenthesized expressions"
      $ do
...
testIntLiteralParser = context "integer literals"
  $ do
    it "accepts the integer literal zero"
      $ do
        "0" `shouldParse` IR.IntLiteral NoSrcSpan 0 Nothing
    it "accepts zero with sign"
      $ do
        "+0" `shouldParse` IR.IntLiteral NoSrcSpan 0 Nothing
        "-0" `shouldParse` IR.IntLiteral NoSrcSpan 0 Nothing
    it "accepts decimal integer literals"
      $ do
        "42" `shouldParse` IR.IntLiteral NoSrcSpan 42 Nothing
    it "accepts decimal integer literals with sign"
      $ do
        "+42" `shouldParse` IR.IntLiteral NoSrcSpan 42 Nothing
        "-42" `shouldParse` IR.IntLiteral NoSrcSpan (-42) Nothing
    it "accepts decimal integer literals with leading zeros"
      $ do
        "007" `shouldParse` IR.IntLiteral NoSrcSpan 7 Nothing
    it "decimal integer literals with leading zeros are not octal"
      $ do
        "009" `shouldParse` IR.IntLiteral NoSrcSpan 9 Nothing
    it "accepts octal integer literals"
      $ do
        "0o644" `shouldParse` IR.IntLiteral NoSrcSpan 0o644 Nothing
    it "octal integer literal prefix is case insensitive"
      $ do
        "0O644" `shouldParse` IR.IntLiteral NoSrcSpan 0o644 Nothing
    it "accepts octal integer literals with sign"
      $ do
        "+0o755" `shouldParse` IR.IntLiteral NoSrcSpan 0o755 Nothing
        "-0o777" `shouldParse` IR.IntLiteral NoSrcSpan (-0o777) Nothing
    it "accepts hexadecimal integer literals"
      $ do
        "0x2A" `shouldParse` IR.IntLiteral NoSrcSpan 0x2A Nothing
    it "hexadecimal integer literal prefix is case insensitive"
      $ do
        "0X2A" `shouldParse` IR.IntLiteral NoSrcSpan 0x2A Nothing
    it "hexadecimal integer literal is case insensitive"
      $ do
        "0x2a" `shouldParse` IR.IntLiteral NoSrcSpan 0x2A Nothing
    it "accepts hexadecimal integer literals with sign"
      $ do
...
testFuncDeclParser = context "function declarations"
  $ do
    let a           = IR.TypeVar NoSrcSpan "a"
        ...
        yPatStrict  = IR.VarPat NoSrcSpan "y" Nothing True
        x           = IR.Var NoSrcSpan (IR.UnQual (IR.Ident "x")) Nothing
    it "accepts function declarations without arguments"
      $ do
        "f = x" `shouldParse` IR.FuncDecl NoSrcSpan f [] [] Nothing x
    it "accepts function declarations with a single argument"
      $ do
        "f x = x" `shouldParse` IR.FuncDecl NoSrcSpan f [] [xPat] Nothing x
    it "accepts function declarations with a single strict argument"
      $ do
        "f !x = x"
          `shouldParse` IR.FuncDecl NoSrcSpan f [] [xPatStrict] Nothing x
    it "accepts function declarations with multiple arguments"
      $ do
        "f x y = x"
          `shouldParse` IR.FuncDecl NoSrcSpan f [] [xPat, yPat] Nothing x
    it "accepts function declarations with multiple strict arguments"
      $ do
        "f !x !y = x"
          `shouldParse` IR.FuncDecl NoSrcSpan f [] [xPatStrict, yPatStrict]
          Nothing x
    it "accepts function declarations with a type-annotated argument"
      $ do
        "f (x :: a) = x"
          `shouldParse` IR.FuncDecl NoSrcSpan f [] [xPat'] Nothing x
    it "accepts function declarations with a strict type-annotated argument"
      $ do
        "f !(x :: a) = x"
          `shouldParse` IR.FuncDecl NoSrcSpan f [] [xPatStrict'] Nothing x
    it "accepts function declarations with annotated return type"
      $ do
        "f x :: a = x"
          `shouldParse` IR.FuncDecl NoSrcSpan f [] [xPat] (Just a) x
    it "accepts nullary function declarations with annotated return type"
      $ do
        "f :: a = x" `shouldParse` IR.FuncDecl NoSrcSpan f [] [] (Just a) x
    it "accepts function declarations with type arguments"
      $ do
        "f @a = x" `shouldParse` IR.FuncDecl NoSrcSpan f [aDecl] [] Nothing x
    it "accepts function declarations with arguments and type arguments"
      $ do
        "f @a x = x"
          `shouldParse` IR.FuncDecl NoSrcSpan f [aDecl] [xPat] Nothing x
    it "accepts function declarations with qualified names"
      $ do
...
testImportDeclParser = context "import declarations"
  $ do
    it "accepts import declarations"
      $ do
        "import M" `shouldParse` IR.ImportDecl NoSrcSpan "M"
    it "accepts import declarations for dotted module names"
      $ do
...
testModuleParser = context "modules"
  $ do
    ...
    it "accepts modules with type signatures"
      $ do
        shouldParseModule ["module M where", "foo :: Foo"] emptyModule
          { IR.modName     = "M"
          , IR.modTypeSigs = [ IR.TypeSig NoSrcSpan [funFoo]
                                 (IR.TypeScheme NoSrcSpan [] tyFoo)
                             ]
          }
    it "accepts modules with function declarations"
      $ do
    ...
    it ("accepts modules with nullary function declarations whose return type"
      ...
      $ do
        shouldParseModule ["module M where", "foo :: Foo = x"] emptyModule
          { IR.modName      = "M"
          , IR.modFuncDecls
              = [IR.FuncDecl NoSrcSpan funFoo [] [] (Just tyFoo) x]
          }
    it "accepts modules with top-level declarations separated by semicolon"
      $ do
