...
testFreshIdentifiers = describe "FreeC.Environment.Fresh"
  $ do
    context "freshCoqIdent"
      $ do
        it "avoids collision with other fresh identifiers"
          $ shouldSucceedWith
          $ do
            x0 <- freshCoqIdent "x"
            x1 <- freshCoqIdent "x"
            return ((x0, x1) `shouldBe` ("x_0", "x_1"))
        it "avoids collision with user defined identifiers"
          $ shouldSucceedWith
          $ do
        ...
        it "is affected by local environments"
          $ shouldSucceedWith
          $ do
            x1 <- localEnv
              $ do
