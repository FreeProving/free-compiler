...
testConvertExpr = describe "FreeC.Backend.Coq.Converter.Expr.convertExpr"
  $ do
    testConvertConApp
    testConvertFuncApp
    testConvertIf
...
testConvertConApp = context "constructor applications"
  $ do
    ...
    it "converts constructor applications correctly"
      $ shouldSucceedWith
      $ do
        "D" <- defineTestTypeCon "D" 0 ["C"]
    ...
    it "requires visible type applications of constructors"
      $ do
...
testConvertFuncApp = context "function applications"
  $ do
    ...
    it "requires visible type applications of functions"
      $ do
...
testConvertIf :: Spec
testConvertIf = context "if expressions"
  $ do
    it "converts if expressions correctly"
      $ shouldSucceedWith
      $ do
...
testConvertCase = context "case expressions"
  $ do
    ...
    it "uses data (not smart) constructors"
      $ shouldSucceedWith
      $ do
        "e" <- defineTestVar "e"
        "e1" <- defineTestVar "e1"
...
testConvertLambda = context "lambda abstractions"
  $ do
...
testConvertExprTypeAnnotations = context "type annotations"
  $ do
    it "ignores expression type annotations"
      $ shouldSucceedWith
      $ do
...
testConvertTypeAppExprs = context "visible type applications"
  $ do
...
testConvertInteger = context "integer expressions"
  $ do
...
testConvertUndefined = context "undefined expressions"
  $ do
    it "translates undefined expressions correctly"
      $ shouldSucceedWith
      $ do
        "a" <- defineTestTypeVar "a"
...
testConvertError = context "error expressions"
  $ do
    it "translates error expressions correctly"
      $ shouldSucceedWith
      $ do
        "a" <- defineTestTypeVar "a"
