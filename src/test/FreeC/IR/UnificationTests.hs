...
testUnification = describe "FreeC.IR.Unification.unify" $ do
  ...
  context "type constructors" $ do
    it "constructors match the same constructor" $ shouldSucceedWith $ do
      t <- parseTestType "()"
      ( t, t ) `shouldUnifyTo` t
    it "arguments of constructors are matched recursively" $ shouldSucceedWith
      $ do
  ...
  context "type synonyms" $ do
    ...
    it "expands type synonyms with arguments when necessary" $ shouldSucceedWith
      $ do
  ...
  context "QuickCheck" $ do
    it "self-unification yields the identity substitution" $ property
...
shouldFailUnification t s = do
  ...
  case res of
    ...
    Left (OccursCheckFailure _ _) -> return $ expectationFailure
      $ "Expected unification error, but occurs check failed for `"
      ++ showPretty t ++ "` and `" ++ showPretty s ++ "`."
    Left (RigidTypeVarError _ _ _) -> return $ expectationFailure
      $ "Expected unification error, but unification failed due to "
      ++ "matching of a rigid type variable in unification of `" ++ showPretty t
      ++ "` and `" ++ showPretty s ++ "`."
    Right mgu -> return $ expectationFailure
      $ "Expected unification error, but found unificator " ++ showPretty mgu
      ++ " for `" ++ showPretty t ++ "` and `" ++ showPretty s ++ "`."
...
shouldFailOccursCheck t s = do
  ...
  case res of
    ...
    Left (UnificationError _ _) -> return $ expectationFailure
    ...
    Left (RigidTypeVarError _ _ _) -> return $ expectationFailure
      ...
      ++ "` and `" ++ showPretty s ++ "`."
    Right mgu -> return $ expectationFailure
      $ "Expected occurs check to fail, but found unificator " ++ showPretty mgu
      ++ " for `" ++ showPretty t ++ "` and `" ++ showPretty s ++ "`."
