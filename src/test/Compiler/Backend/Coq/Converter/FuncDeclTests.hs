module Compiler.Backend.Coq.Converter.FuncDeclTests where
...
testConvertNonRecFuncDecl =
  describe "Compiler.Backend.Coq.Converter.FuncDecl.convertNonRecursiveFunction"
    $ do
...
testConvertRecFuncDeclsWithHelpers =
  describe
      "Compiler.Backend.Coq.Converter.FuncDecl.convertRecFuncDeclsWithHelpers"
    $ do
        ...
        it "translates partial recursive functions correctly"
          ...
          ++ "           if cond_0"
          ++ "           then x_1"
          ++ "           else xs'_1 >>= (fun (xs'_2 : List Shape Pos a_0) =>"
          ++ "                      findJust_1 P xs'_2))"
          ++ "     end. "
        ...
        it
          ...
          ++ "        ((fun (ys_1 : Free Shape Pos (List Shape Pos a)) =>"
          ++ "            xs' >>= (fun (xs'_0 : List Shape Pos a) =>"
          ++ "              @append_0 Shape Pos a xs'_0 ys_1)) ys)"
          ++ "    end. "
        ...
        it
          ...
          $ do
              ...
              shouldTranslateDeclsTo
                ...
                ++ "        (pure (fun (y_1 : Free Shape Pos a) => xs' >>="
                ++ "           (fun (xs'_0 : List Shape Pos a) =>"
                ++ "             @foo_0 Shape Pos a xs'_0 y_1))))"
                ++ "    end. "
                ++ "Definition foo (Shape : Type) (Pos : Shape -> Type) {a : Type}"
                ++ "  (xs : Free Shape Pos (List Shape Pos a))"
                ++ " : Free Shape Pos (List Shape Pos a)"
        ...
        it
          ...
          $ do
              shouldTranslateDeclsTo
                ...
                ++ "    | nil => @Nil Shape Pos a"
                ++ "    | cons x xs' =>"
                ++ "        (fun (n_3 : Free Shape Pos (Integer Shape Pos)) =>"
                ++ "           xs' >>= (fun (xs'_0 : List Shape Pos a) =>"
                ++ "             @take_0 Shape Pos a"
                ++ "               n_3"
                ++ "               xs'_0))"
...
testConvertRecFuncDeclsWithSections = do
  describe
      "Compiler.Backend.Coq.Converter.FuncDecl.convertRecFuncDeclsWithSections"
    $ do
        it "creates variable sentences for constant arguments"
          ...
          ++ "        @Cons Shape Pos b_0"
          ++ "          (f_0 >>= (fun f_1 => f_1 x_1))"
          ++ "          (xs'_1 >>= (fun (xs'_2 : List Shape Pos a_0) =>"
          ++ "             map_1 xs'_2))"
          ++ "     end. "
        ...
        it
          ...
          ++ "        @Cons Shape Pos b_0"
          ++ "          (f_0 >>= (fun f_1 => f_1 x_2))"
          ++ "          (xs'_2 >>= (fun (xs'_4 : List Shape Pos a_0) =>"
          ++ "             mapAlt'_1 xs'_4))"
          ++ "     end"
          ...
          ++ "        @Cons Shape Pos b_0"
          ++ "          (f_0 >>= (fun f_1 => f_1 x_3))"
          ++ "          (xs'_3 >>= (fun (xs'_4 : List Shape Pos a_0) =>"
          ++ "             mapAlt_1 xs'_4))"
          ++ "     end. "
        ...
        it "passes constant type arguments explicitly the main function"
          ...
          ++ "          (xs'_1 >>= (fun (xs'_2 : List Shape Pos c_0) =>"
          ++ "                       foo_1 xs'_2))"
          ++ "     end. "
          ++ "(* Main functions for foo *) "
