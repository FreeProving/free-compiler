  ...
  it "translates mutually recursive functions correctly"
    ...
    $ do
        ...
        shouldConvertWithHelpersTo
          ...
          ++ "Fixpoint even_len0 (Shape : Type) (Pos : Shape -> Type) {a : Type}"
          ++ "  (xs : List Shape Pos a)"
          ++ "  {struct xs}"
          ++ "  := match xs with"
          ++ "     | nil        => True Shape Pos"
  ...
  it "allows the arguments of helper functions to be shadowed"
    ...
    $ do
        ...
        shouldConvertWithHelpersTo
          ...
          ++ "        (xs0 >>= (fun (xs1 : List Shape Pos a) =>"
          ++ "             @length0 Shape Pos a xs1))"
          ++ "    end. "
          ++ "Definition length (Shape : Type) (Pos : Shape -> Type) {a : Type}"
          ++ "  (xs : Free Shape Pos (List Shape Pos a))"
          ++ "  : Free Shape Pos (Integer Shape Pos)"
  ...
  it
    ...
    $ do
        ...
        shouldConvertWithHelpersTo
          ...
          ++ "                 (xs' >>= (fun (xs'0 : List Shape Pos a) =>"
          ++ "                   @append0 Shape Pos a (Unit Shape Pos) xs'0 ys))"
          ++ "                 y)"
          ++ "            (Tt Shape Pos))"
          ++ "     end."
          ++ " Definition append"
