  ...
  = describe "FreeC.Backend.Coq.Converter.TypeDecl.convertTypeSynDecl" $ do
    ...
    it "expands type synonyms in mutually recursive data type declarations"
      ...
      $ do
        ...
        shouldConvertLocalTypeDeclsTo
          ...
          ++ "        (x3 : List Shape Pos (Tree Shape Pos a0)) "
          ...
          ++ "        (x2 : List Shape Pos (Tree Shape Pos a0))"
          ++ "        : Free Shape Pos (List Shape Pos (Tree Shape Pos a0))"
          ++ "      := match x2 with "
          ++ "         | nil => pure nil "
          ++ "         | cons x3 x4 => "
          ++ "             cbneed Shape Pos shareArgsTree_ x3 >>= (fun sx1 =>"
          ++ "             cbneed Shape Pos shareArgsListTree_ x4 >>="
          ++ "             (fun sx2 => "
          ++ "             pure (cons sx1 sx2))) "
          ++ "         end "
          ++ "    in match x with "
          ++ "       | leaf x0 => cbneed Shape Pos shareArgs x0 >>= (fun sx =>"
          ++ "                    pure (leaf sx)) "
          ++ "       | branch x1 => "
          ++ "           cbneed Shape Pos shareArgsListTree_ x1 >>="
          ++ "           (fun sx0 =>"
          ++ "           pure (branch sx0)) "
          ++ "       end. "
    ...
    it "sorts type synonym declarations topologically" $ shouldSucceedWith $ do
      ...
      shouldConvertLocalTypeDeclsTo
        ...
        ++ "  : Free Shape Pos (Foo Shape Pos)"
        ++ " := let 'foo x0 x1 := x"
        ++ "    in cbneed Shape Pos shareArgsFoo x0 >>= (fun sx =>"
        ++ "       cbneed Shape Pos shareArgsFoo x1 >>= (fun sx0 =>"
        ++ "       pure (foo sx sx0))). "
    ...
    context "Translation of types and type synonyms" $ do
      ...
      it "translates polymorphic data types correctly" $ shouldSucceedWith $ do
        ...
        shouldConvertLocalTypeDeclsTo
          ...
          ++ "    | baz x1 => nf x1 >>= (fun nx0 => pure (baz (pure nx0)))"
          ++ "    end. "
          ...
          ++ " := match x with"
          ++ "    | bar x0 => cbneed Shape Pos shareArgs x0 >>= (fun sx =>"
          ++ "                pure (bar sx))"
          ++ "    | baz x1 => cbneed Shape Pos shareArgs x1 >>= (fun sx0 =>"
          ++ "                 pure (baz sx0))"
          ++ "    end. "
      ...
      it "renames type variables with same name as generated constructors"
        ...
        $ do
          ...
          shouldConvertLocalTypeDeclsTo (NonRecursive "data Foo a = A a")
            ...
            ++ " := let 'a x0 := x"
            ++ "    in cbneed Shape Pos shareArgs x0 >>= (fun sx =>"
            ++ "       pure (a sx)). "
      ...
      it "translates mutually recursive data types correctly"
        ...
        $ do
          ...
          shouldConvertLocalTypeDeclsTo
            ...
            ++ "  : Free Shape Pos (Foo Shape Pos)"
            ++ " := let 'foo x0 := x"
            ++ "    in cbneed Shape Pos shareArgsBar x0 >>= (fun sx =>"
            ++ "       pure (foo sx)) "
            ++ "with "
            ++ "shareArgsBar"
            ++ "  {Shape : Type} {Pos : Shape -> Type}"
    ...
    context "Generation of induction schemes" $ do
      it "creates a correct induction scheme" $ shouldSucceedWith $ do
        ...
        shouldConvertLocalTypeDeclsTo
          ...
          ++ "  : Free Shape Pos (Foo Shape Pos a)"
          ++ " := let 'foo x0 x1 x2 := x "
          ++ "    in cbneed Shape Pos shareArgsFoo_ x0 >>= (fun sx =>"
          ++ "       cbneed Shape Pos shareArgs x1 >>= (fun sx0 =>"
          ++ "       cbneed Shape Pos shareArgsFoo_ x2 >>= (fun sx1 =>"
          ++ "       pure (foo sx sx0 sx1)))). "
          ++ "Instance ShareableArgsFoo_"
          ++ "  {Shape : Type} {Pos : Shape -> Type} {a : Type}"
          ++ "  `{Injectable Share.Shape Share.Pos Shape Pos}"
