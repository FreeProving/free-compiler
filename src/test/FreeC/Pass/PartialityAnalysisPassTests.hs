...
shouldBePartial = shouldBePartialWith $ \funcName partial -> if partial
  then return () else expectationFailure $ "Expected " ++ showPretty funcName
...
testPartialityAnalysisPass = describe "FreeC.Pass.PartialityAnalysisPass" $ do
  it "does not classify non-partial functions as partial" $ shouldSucceedWith
    $ do
      _ <- defineTestFunc "maybeHead" 1 "forall a. ([]) a -> Maybe a"
      shouldNotBePartial $ NonRecursive $ "maybeHead xs = case xs of {"
        ++ "  ([])      -> Nothing;" ++ "  (:) x xs' -> Just x" ++ "}"
  it "recognizes directly partial functions using 'undefined'"
    $ shouldSucceedWith $ do
      _ <- defineTestFunc "head" 1 "forall a. ([]) a -> a"
      shouldBePartial $ NonRecursive $ "head xs = case xs of {"
        ++ "  ([])      -> undefined;" ++ "  (:) x xs' -> x" ++ "}"
  it "recognizes directly partial functions using 'error'" $ shouldSucceedWith
    $ do
      _ <- defineTestFunc "head" 1 "forall a. ([]) a -> a"
      shouldBePartial $ NonRecursive $ "head xs = case xs of {"
        ++ "  ([])      -> error \"head: empty list\";" ++ "  (:) x xs' -> x"
        ++ "}"
  ...
  it "recognizes mutually recursive partial functions" $ shouldSucceedWith $ do
    ...
    shouldBePartial $ Recursive
      [ "pairs xys = case xys of {" ++ "    ([])     -> ([]);"
          ++ "    (:) x ys -> pairs' x ys" ++ "  }"
      , "pairs' x yxs = case yxs of {" ++ "    ([])     -> undefined;"
          ++ "    (:) y xs -> (:) ((,) x y) (pairs xs)" ++ "  }"
      ]
