\section{Expressions} \label{sec:translation:expr}
...
\subsection{Variables} \label{sec:translation:expr:vars}
...
\subsection{Function Applications} \label{sec:translation:expr:app}
...
According to the \ref{eqn:translation:expr:invariant}, the expression \haskell{Just} should now be translated to a Coq term of type
\[
...
But before we examine the translation of lambda abstractions next, let's first look into another kind of function application: operator applications.
\begin{figure}[H]
  \begin{align*}
...
When used properly, this notation can often increase the readability of programs.
...
A left section coincides with a regular partial application of the operator.
A right section on the other hand cannot be expressed in this way.
...
\subsection{\haskell{case} and \haskell{if} Expressions}
...
\]
For simplicity, let's first focus on the translation of \haskell{if} expressions.
...
\subsection{Error Terms} \label{sec:translation:expr:errors}
...
How $\Shape$ and $\Pos$ would have to look like in case of \haskell{Maybe}, has been shown in \autoref{sec:preliminaries:modeling-effects:containers} already.
...
\subsection{Literals} \label{sec:translation:expr:literals}
