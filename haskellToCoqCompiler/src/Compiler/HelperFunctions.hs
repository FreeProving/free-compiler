...
getMatchFromTerm (G.Match mItem retType equations) =
  G.Match mItem retType equations
getMatchFromTerm (G.App cons args) = if containsMatchTerm cons
  then getMatchFromTerm cons
  else getMatchFromTerm
    (head (filter containsMatchTerm (map argToTerm (fromNonEmptyList args))))
getMatchFromTerm (G.If _ cond _ tTerm eTerm) = if containsMatchTerm cond
...
getConstrNamesFromDataDecls
  :: Show l => [H.Decl l] -> [[(G.Qualid, Maybe G.Qualid)]]
...
getConstrNamesFromDataDecl
  :: Show l => H.Decl l -> [H.Decl l] -> [(G.Qualid, Maybe G.Qualid)]
...
getConstrsByName
  :: Show l => G.Name -> [H.Decl l] -> [(G.Qualid, Maybe G.Qualid)]
...
getConstrsByPattern (G.QualidPat qId) dataTypeConstrs =
  if (not . null) filteredConstrs then Just (head filteredConstrs) else Nothing
 where
  filteredConstrs =
    filter (not . null) (map (getConstrSet qId) dataTypeConstrs)
...
getConstrSet qId constrs = if any (eqQId qId) constrs then constrs else []

getConstrNameFromQConDecl
  :: Show l => H.QualConDecl l -> G.Name -> (G.Qualid, Maybe G.Qualid)
getConstrNameFromQConDecl (H.QualConDecl _ _ _ (H.ConDecl _ conName _)) funName
  ...
  where conStr = nameToStr conName
...
getTypeNamesFromTerm (G.App term args) =
  map strToGName (termToStrings term) ++ map
    strToGName
    (concatMap (termToStrings . argToTerm) (fromNonEmptyList args))
...
getBinderByQId (x : xs) qId = if eqQId qId (termToQId (getBinderName x))
  then Just x
...
containsAllConstrs (p : ps) constrs = containsAllConstrs
  ps
  (filter (not . eqQId constrQId) constrs)
  where constrQId = getQIdFromPattern p
...
containsRecursiveCall (G.App term args) funName =
  containsRecursiveCall term funName
    || or [ containsRecursiveCall t funName | t <- argTerms ]
  where argTerms = convertArgumentsToTerms (fromNonEmptyList args)
...
containsRecursiveCall (G.If _ cond _ thenTerm elseTerm) funName =
  containsRecursiveCall cond funName
    || containsRecursiveCall thenTerm funName
...
containsMatchTerm (G.App term args) = containsMatchTerm term
  || any containsMatchTerm (map argToTerm (fromNonEmptyList args))
...
eqBinder lBinder rBinder = eqQId ((termToQId . getBinderName) lBinder)
                                 ((termToQId . getBinderName) rBinder)
...
changeSimilarType qId =
  if any (eqQId qId) haskellTypes then strToQId (qIdToStr qId ++ "_") else qId
...
filterEachElement (x : xs) f list = filterEachElement xs f filteredList
  where filteredList = filter (f x) list
...
addInferredTypesToSignature binders dataNames retType =
 ...
 where
  filteredTypeNames =
    nub
      (filterEachElement dataNames
                         dataTypeUneqGName
                         (filter isCoqType typeNames)
      )
...
getInferredBindersFromRetType (G.App _ args) = concatMap
...
combineArgs args (G.App term args') = combineArgs combArgs term
  where combArgs = fromNonEmptyList args' ++ args
combineArgs args _ =
...
applyToDeclHeadTyVarBinds
  :: Show l => H.DeclHead l -> (H.TyVarBind l -> a) -> [a]
...
termToStrings (G.App term args) =
  termToStrings term ++ listToStrings argToStrings (fromNonEmptyList args)
...
termToStrings (G.Arrow term1 term2) =
  termToStrings term1 ++ termToStrings term2
...
multPatToStrings (G.MultPattern (pat B.:| pats)) = listToStrings patToStrings
                                                                 patList
  where patList = pat : pats
...
haskellTypes =
  [strToQId "Bool", strToQId "Option", strToQId "Identity", strToQId "Nat"]
