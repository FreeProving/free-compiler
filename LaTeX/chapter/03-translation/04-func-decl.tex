\section{Function Declarations} \label{sec:translation:func-decl}
...
\subsection{Recursive Functions} \label{sec:translation:func-decl:rec}
...
\begin{minted}{haskell}
  ...
  append xs ys = case xs of
    ...
    x : xs' -> x : append xs' ys
\end{minted}
The reason for this error is that \coq{Definition} sentences are not allowed to reference themselves in Coq.
...
In much the same way as shown for \haskell{undefined} in \autoref{sec:translation:expr:errors}, \haskell{loop} could therefore be used to prove arbitrary propositions when translated to Coq.
...
In the example above, \coq{append} is decreasing on its first explicit argument, namely \coq{xs}.
It is decreasing on this argument because in the only recursive call \coq{append xs' ys} the first argument, i.e., \coq{xs'}, is bound to a subterm of \coq{xs} by the means of pattern matching.
...
Let's first recall the definition of the monadically lifted \coq{List} type from \autoref{sec:translation:type-decl:rec}.
...
Of particular interest are the first two steps.
...
Therefore, we have to consider every possible combination of decreasing arguments and test whether all functions in the strongly connected component actually decrease on those arguments.
...
The same is true when the \haskell{case} expression is wrapped by a lambda abstraction.
...
\subsection{Partial Functions} \label{sec:translation:func-decl:partial}
...
In case of recursive functions, the main and helper functions need a \coq{Partial} instance if and only if the original function needs a \coq{Partial} instance.
...
The only function invoked by \haskell{map}~--~except for constructors and itself~--~is the function passed as its first argument.
\begin{minted}{haskell}
  map :: (a -> b) -> [a] -> [b]
  map f xs = case xs of
    []      -> []
