...
parseAndSimplifyExpr = (parseHaskell . mkSrcFile "<test-input>") >=> simplifyExpr
...
testSimplifyExpr = context "simplifyExpr" $ do
  it "simplifies single variable pattern case expression to lambda abstractions" $ shouldSucceedWith $ do
