\section{Type Declarations} \label{sec:translation:type-decl}
...
\subsection{Type Synonym Declarations} \label{sec:translation:type-decl:type-syn}
Type synonyms can be used in Haskell to give a name to a more complex type expression.
Similarly, we can use a \coq{Definition}-sentence to assign a name to a term in Coq.
...
\begin{minted}{haskell}
  type Queue a = List a
\end{minted}
needs $\Shape$ and $\Pos$ when translated to Coq because they need to be passed to \coq{List}.
\begin{minted}{coq}
...
\subsection{Data Type Declarations} \label{sec:translation:type-decl:data}
...
With that end in view, we define a function for each regular constructor that simply applies the constructor and lifts its result using the \coq{pure} constructor of the free monad.
...
Generating \coq{Arguments}-sentence is not necessary as we can mark the type argument as implicit directly in the \coq{Definition}-sentence by wrapping its binder in curly braces.
...
However, the approach above fails in case of \textit{mutually recursive data types}.
\begin{minted}{haskell}
...
The \coq{with} keyword is used to concatenate multiple \coq{Inductive}-sentences to a single sentence.
...
\haskell{Tree} is translated to an \coq{Inductive}-sentence while \haskell{Forest} is translated to a \coq{Definition}-sentence.
Unfortunately, \coq{with} can only be used to join two or more \coq{Inductive}-sentences.
