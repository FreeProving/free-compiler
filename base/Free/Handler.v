...
Fixpoint runNDSharing A {Shape : Type} {Pos : Shape -> Type} 
  ...
  : Free (Comb.Shape ND.Shape Shape) (Comb.Pos ND.Pos Pos) A  :=
      ...
      | impure (inl Share.sget) pf => nameChoices next scope scopes (pf n)
      | impure (inl (Share.sput n')) pf => nameChoices next n' scopes (pf tt) 
      | impure (inr (inl (ND.schoice id))) pf =>               
              let l := nameChoices (next + 1) scope scopes (pf (true : Comb.Pos Share.Pos (Comb.Pos ND.Pos Pos) (inr (inl (ND.schoice id))))) in
              let r := nameChoices (next + 1) scope scopes (pf false) in
              Choice (Some (tripl scope next)) l r
      | impure (inr (inl ND.sfail)) _ => Fail

      | impure (inr (inr s)) pf => impure (inr s) (fun p => nameChoices next scope scopes (pf p)) 
  ...
  in match fs with (* outside scope handler *)
     ...
     | impure (inl (Share.sesharing n'))  pf => runNDSharing n' (pf tt) (* error *)
     | impure (inl Share.sget) pf => runNDSharing n (pf n)
     | impure (inl (Share.sput n')) pf => runNDSharing n' (pf tt)
     | impure (inr s) pf => impure s (fun p => runNDSharing n (pf p))
     end.
