...
convertRecFuncDeclsWithSectionTo
  :: [ String ] -> String -> Converter Expectation
    ...
    $ do
      ...
      convertRecFuncDeclsWithSectionTo
        [ "map @a @b (f :: a -> b) (xs :: List a) :: List b"
            ++ "  = case xs of {" ++ "      Nil        -> Nil @b;"
            ++ "      Cons x xs' -> Cons @b (f x) (map @a @b f xs')" ++ "    }"
        ] $ "Section section_map_0. " ++ "(* Constant arguments for map *) "
        ++ "Variable Shape : Type. " ++ "Variable Pos : Shape -> Type. "
        ++ "Variable a_0 b_0 : Type. " ++ "Variable f_0 : Free Shape Pos"
        ++ "  (Free Shape Pos a_0 -> Free Shape Pos b_0). "
        ++ "(* Helper functions for map *) "
        ++ "Fixpoint map_1 (xs : List Shape Pos a_0) {struct xs}"
        ++ "  := match xs with" ++ "     | nil            => @Nil Shape Pos b_0"
        ++ "     | cons x xs' =>" ++ "        @Cons Shape Pos b_0"
        ++ "          (f_0 >>= (fun f_1 => f_1 x))"
  ...
  it "does not create variable sentences for unused constant arguments"
    $ shouldSucceedWith $ do
      ...
      convertRecFuncDeclsWithSectionTo
        ...
        ] $ "Section section_foo_0. " ++ "(* Constant arguments for foo *) "
        ++ "Variable Shape : Type. " ++ "Variable Pos : Shape -> Type. "
        ++ "Variable a_0 : Type. " ++ "Variable v_0 : Free Shape Pos a_0. "
        ++ "(* Helper functions for foo *) "
        ++ "Fixpoint foo_1 (xs : List Shape Pos a_0) {struct xs}"
        ++ "  := match xs with" ++ "     | nil            => @Nil Shape Pos a_0"
        ++ "     | cons x xs' =>" ++ "        @Cons Shape Pos a_0 v_0"
        ++ "          (xs' >>= (fun (xs'_0 : List Shape Pos a_0) =>"
        ++ "              foo_1 xs'_0))" ++ "     end. "
  ...
  it "passes constant type arguments explicitly the main function"
    $ shouldSucceedWith $ do
      ...
      convertRecFuncDeclsWithSectionTo
        ...
        ++ "Variable Shape : Type. " ++ "Variable Pos : Shape -> Type. "
        ++ "Variable b_0 : Type. " ++ "Variable v_0 : Free Shape Pos b_0. "
        ++ "(* Helper functions for foo *) "
        ++ "Fixpoint foo_1 {a_0 c_0 : Type}"
        ++ "  (xs : List Shape Pos c_0) {struct xs}" ++ "  := match xs with"
        ++ "     | nil            => @Nil Shape Pos b_0"
        ++ "     | cons x xs' =>" ++ "        @Cons Shape Pos b_0 v_0"
