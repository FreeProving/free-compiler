...
testTypeSynExpansion = describe "FreeC.IR.TypeSynExpansion"
  $ do
...
testExpandTypeSynonym = context "expandTypeSynonym"
  $ do
    it "expands only the outermost type synonym"
      $ shouldSucceedWith
      $ do
    ...
    it "does not expand type synonyms recursively"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeSyn "Foo" ["a"] "Bar a"
        ...
        output <- expandTypeSynonym input
        return (output `shouldBeSimilarTo` expectedOutput)
    it "does not expand nested type synonyms"
      $ shouldSucceedWith
      $ do
...
testExpandAllTypeSynonyms = context "expandAllTypeSynonyms"
  $ do
    it "expands outermost type synonyms"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeSyn "Foo" ["a"] "Bar a"
        _ <- defineTestTypeCon "Bar" 1 []
        ...
        return (output `shouldBeSimilarTo` expectedOutput)
    it "expands type synonyms recursively"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeSyn "Foo" ["a"] "Bar a"
        _ <- defineTestTypeSyn "Bar" ["a"] "Baz a"
        ...
        output <- expandAllTypeSynonyms input
        return (output `shouldBeSimilarTo` expectedOutput)
    it "expands nested type synonyms"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 1 []
        _ <- defineTestTypeSyn "Bar" [] "Baz"
