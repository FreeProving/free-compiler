...
In case of \coq{Just} we were able to manully choose appropriate names for the arguments of the smart constructor.
...
If we convert the example to Coq, the declaration of \coq{B} must preceed the declaration of \coq{A}.
\begin{minted}{coq}
...
As we have seen in case of ordninary recursive data types like \coq{List}, Coq allows us to use the data type defined by an \coq{Inductive}-sentence within that sentence.
...
\haskell{Tree} and \haskell{Forest} on the other hand definitly need to be declared in the same sentence, as the corresponding nodes are reachable from each other.
...
\begin{align*}
  &\left[\{\, \haskellM{B} \,\}, \{\, \haskellM{A} \,\}\right] \\
  &\left[\{\, \haskellM{List} \,\}\right]                      \\
  &\left[\{\, \haskellM{Tree}, \haskellM{Forest} \,\}\right]   \\
\end{align*}
...
\begin{figure}[H]
  ...
  \caption{Dependency graphs for \haskell{Tree} and \haskell{Forest} before inlining the type synonym \haskell{Forest} into the definition of \haskell{Tree} (left) and after inlining (right).}
