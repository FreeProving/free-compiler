...
checkPatternFuncDecl funcDecl = checkPatternExpr (IR.funcDeclRhs funcDecl)
 where
  ...
  checkPatternExpr (IR.Case srcSpan exprScrutinee exprAlts _) = do
    ...
    case IR.exprType exprScrutinee >>= getTypeConName of
      ...
      Just typeName -> do
        ...
        case maybeEntry of
          Just entry -> unfoldTypeSyn entry altConNames srcSpan
  ...
  -- | converts type synonyms and performs check on data entries
  unfoldTypeSyn :: EnvEntry -> [IR.ConName] -> SrcSpan -> Converter ()
  unfoldTypeSyn entry altConNames srcSpan | isDataEntry entry = do
    performCheck (entryConsNames entry) altConNames srcSpan
  unfoldTypeSyn entry altConNames srcSpan | isTypeSynEntry entry = do
    case getTypeConName (entryTypeSyn entry) of
      Just typeConName -> do
        maybeEntry <- inEnv $ lookupEntry IR.TypeScope typeConName
        case maybeEntry of
          Just entry' -> unfoldTypeSyn entry' altConNames srcSpan
          _           -> failedPatternCheck srcSpan
      _ -> failedPatternCheck srcSpan
  unfoldTypeSyn _ _ srcSpan = failedPatternCheck srcSpan
