...
module FreeC.Backend.Coq.Converter.TypeDecl.InductionScheme  ( generateInductionScheme ) where
...
generateInductionScheme (IR.DataDecl _ (IR.DeclIdent _ name) typeVarDecls conDecls) = localEnv $ do
