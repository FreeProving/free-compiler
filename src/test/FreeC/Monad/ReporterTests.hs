...
testReporter :: Spec
testReporter = describe "FreeC.Monad.Reporter"
  $ do
    testRunReporter
    testIsFatal
    testMessages
...
testRunReporter = describe "runReporter"
  $ do
    it "returns 'Just' the produced value if no message was reported"
      $ do
        runReporter (return testValue) `shouldBe` (Just testValue, [])
    it "returns 'Just' the produced value if no fatal message was reported"
      $ do
        runReporter (report testMessage1 >> return testValue)
          `shouldBe` (Just testValue, [testMessage1])
    it "returns 'Nothing' if a fatal message was reported"
      $ do
...
testIsFatal :: Spec
testIsFatal = describe "isFatal"
  $ do
    it "is not fatal to return from a reporter"
      $ do
        isFatal (return testValue) `shouldBe` False
    it "is not fatal to report a regular message"
      $ do
        isFatal (report testMessage1) `shouldBe` False
    it "is fatal to report a fatal message"
      $ do
        isFatal (reportFatal testMessage1) `shouldBe` True
    it "is fatal if a computation involves reporting a fatal message"
      $ do
...
testLiftIO :: Spec
testLiftIO = describe "liftIO reports IO errors"
  $ do
    it "catches IO errors"
      $ do
