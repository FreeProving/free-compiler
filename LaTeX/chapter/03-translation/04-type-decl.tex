...
For example, we can combine both concepts to define a data type for singly linked lists.
\begin{minted}{haskell}
  data List a = Nil | Cons a (List a)
\end{minted}
In this case, \haskell{List} is also an example for a recursive data type, i.e. values of type \haskell{List} are used to construct new lists.
Data types with more complex depenency relationships will be discussed in the next section but for the translation of \haskell{List} our current approach survices.
...
If we apply this translation rule to our \haskell{List} data type, we obtain the following \coq{Inductive}-sentence for example.
\begin{minted}{coq}
...
The smart constructor for \coq{cons} would look as follows.
\begin{minted}{coq}
