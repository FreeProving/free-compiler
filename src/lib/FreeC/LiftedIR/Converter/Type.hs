...
liftNonRecFuncArgTypes = mapM $ \pat ->
  let err = reportFatal $ Message (IR.varPatSrcSpan pat) Error "Expected Type"
...
liftTypeApp' _ (_ : _) = reportFatal $ Message
  NoSrcSpan
  Error
  "liftTypeApp': Only type constructors can be applied!"
...
markOutermostDecreasing _ = reportFatal
  $ Message NoSrcSpan Error "Outermost argument is not necessarily unique"
