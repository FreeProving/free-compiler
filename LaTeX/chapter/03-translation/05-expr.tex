...
\]
complies with our invariant because $\toCoq{e_2}$ and $\toCoq{e_3}$ both have the type $\toCoq{\tau}$ and therefore the entire term has that type.
Interrestingly, if we applied this naïve translation rule, the generated code works.
...
Their condition can be of any type with exactly two constructors.
The first constructor is interpreted as \textit{truthy} and the other as \textit{falsy}.
Because the type of $\toCoq{e_1}$ is \coq{Free @$\Shape$@ @$\Pos$@ (Bool @$\Shape$@ @$\Pos$@)} and \coq{Free} has exectly two constructors \coq{pure} and \coq{impure}, it can be used in \haskell{if} expressions.
This complies with the results of the listing above.
\haskell{True} and \haskell{False} are values without an effect.
Therefore, their underlying representation uses the \coq{pure} constructor which is interpreted as \textit{truthy}.
\haskell{undefined} on the other hand does have an effect.
The usage of the \coq{impure} constructor leads to \haskell{undefined} being interpreted as \textit{falsy}.
