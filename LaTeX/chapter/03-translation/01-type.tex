\section{Type Expressions} \label{sec:translation:type}
...
\]
Lifting the type of \haskell{xs} to the free monad allows an application of \haskell{map} to pass an effectful computation as the second argument.
...
While it is correct to lift the type of \haskell{f}, i.e., $a \rightarrow b$, into the free monad such that potential effects in the first argument of \haskell{map} can be handled, doing so is not sufficient.
\[
...
In general, the argument and return types of all arbitrarily deeply nested function types in $\tau$ need to be lifted.
