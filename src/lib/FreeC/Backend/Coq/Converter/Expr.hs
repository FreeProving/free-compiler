...
convertLiftedExpr translateEvalArgs
  (LIR.App _ func typeArgs effects args freeArgs) = do
...
convertLiftedExpr _ (LIR.ErrorExpr _)
  = return $ Coq.Qualid Coq.Base.partialError
...
convertExpr' translateEvalArgs = liftExpr
  >=> (convertLiftedExpr translateEvalArgs)
...
convertLiftedAlt translateEvalArgs
  (LIR.Alt _ (LIR.ConPat srcSpan ident) varPats rhs) = do
