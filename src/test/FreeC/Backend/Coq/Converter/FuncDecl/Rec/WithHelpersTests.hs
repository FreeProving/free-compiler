...
testConvertRecFuncDeclWithHelpers = context "with helper functions" $ do
  ...
  it "translates partial recursive functions correctly" $ shouldSucceedWith $ do
    ...
    shouldConvertWithHelpersTo
      ...
      ++ "     | cons x xs' =>" ++ "        (p >>= (fun p_1 => p_1 x)) >>="
      ++ "        (fun (p_1 : Bool Shape Pos) =>" ++ "           if p_1"
      ++ "           then x"
      ...
      ++ "  (p : Free Shape Pos (Free Shape Pos a"
      ++ "    -> Free Shape Pos (Bool Shape Pos)))"
      ++ "  (xs : Free Shape Pos (List Shape Pos a))" ++ "  : Free Shape Pos a"
      ++ "  := xs >>= (fun (xs_0 : List Shape Pos a) =>"
      ++ "               @findJust_0 Shape Pos P a p xs_0)."
  it "translates mutually recursive functions correctly" $ shouldSucceedWith
    $ do
      ...
      shouldConvertWithHelpersTo
        [ "even_len @a (xs :: List a) :: Bool = case xs of {"
            ++ "    Nil -> True;" ++ "    Cons x xs' -> odd_len @a xs'" ++ "  }"
        , "odd_len @a (xs :: List a) :: Bool = case xs of {"
            ++ "    Nil -> False;" ++ "    Cons x xs' -> even_len @a xs'"
            ++ "  }"
        ] $ "(* Helper functions for even_len, odd_len *) "
        ++ "Fixpoint even_len_0 (Shape : Type) (Pos : Shape -> Type) {a : Type}"
        ++ "  (xs : List Shape Pos a)" ++ "  {struct xs}"
        ++ "  := match xs with" ++ "     | nil        => True Shape Pos"
        ++ "     | cons x xs' =>"
        ++ "         xs' >>= (fun (xs'_0 : List Shape Pos a) =>"
        ++ "           @odd_len_0 Shape Pos a xs'_0)" ++ "     end "
        ++ "with odd_len_0 (Shape : Type) (Pos : Shape -> Type) {a : Type}"
        ++ "  (xs : List Shape Pos a)" ++ "  {struct xs}"
        ++ "  := match xs with" ++ "     | nil        => False Shape Pos"
        ++ "     | cons x xs' =>"
        ++ "         xs' >>= (fun (xs'_0 : List Shape Pos a) =>"
        ++ "           @even_len_0 Shape Pos a xs'_0)" ++ "     end. "
  ...
  it "translates recursive functions with outer lambda abstractions correctly"
    $ shouldSucceedWith $ do
      ...
      shouldConvertWithHelpersTo
        ...
        ++ "                  {struct xs}" ++ " := match xs with"
        ++ "    | nil        => ys" ++ "    | cons x xs' => @Cons Shape Pos a x"
        ++ "        ((fun (ys0 : Free Shape Pos (List Shape Pos a)) =>"
        ++ "            xs' >>= (fun (xs'_0 : List Shape Pos a) =>"
        ++ "              @append_0 Shape Pos a xs'_0 ys0)) ys)" ++ "    end. "
  ...
  it ("translates recursive functions with nested pattern matching on"
      ++ "recursive argument correctly") $ shouldSucceedWith $ do
    "List" <- defineTestTypeCon "List" 1 ["Nil", "Cons"]
    ("nil", "Nil") <- defineTestCon "Nil" 0 "forall a. List a"
    ("cons", "Cons") <- defineTestCon "Cons" 2 "forall a. a -> List a -> List a"
    ...
    shouldConvertWithHelpersTo
      ...
      ++ "  (P : Partial Shape Pos) {a : Type}"
      ++ "  (xs : List Shape Pos a) {struct xs}" ++ " := match xs with"
      ++ "    | nil        => @undefined Shape Pos P a" ++ "    | cons x xs' =>"
      ++ "        xs' >>= (fun xs'_0 =>" ++ "            match xs'_0 with"
      ++ "            | nil       => x" ++ "            | cons y ys => ys >>="
      ++ "                 (fun (ys_0 : List Shape Pos a) =>"
      ++ "                    @last_0 Shape Pos P a ys_0)" ++ "            end)"
      ++ "    end. " ++ "Definition last (Shape : Type) (Pos : Shape -> Type)"
      ++ "  (P : Partial Shape Pos) {a : Type}"
      ++ "  (xs : Free Shape Pos (List Shape Pos a))" ++ " : Free Shape Pos a"
      ++ " := xs >>= (fun (xs_0 : List Shape Pos a) =>"
  ...
  it "allows the arguments of helper functions to be shadowed"
    $ shouldSucceedWith $ do
      ...
      shouldConvertWithHelpersTo
        ...
        ++ "        (xs0 >>= (fun (xs_1 : List Shape Pos a) =>"
        ++ "             @length_0 Shape Pos a xs_1))" ++ "    end. "
        ++ "Definition length (Shape : Type) (Pos : Shape -> Type) {a : Type}"
        ++ "  (xs : Free Shape Pos (List Shape Pos a))"
        ++ "  : Free Shape Pos (Integer Shape Pos)"
        ++ " := xs >>= (fun (xs_0 : List Shape Pos a) =>"
  ...
  it "does not pass shadowed arguments of main function to helper functions"
    $ shouldSucceedWith $ do
      ...
      shouldConvertWithHelpersTo
        ...
        ++ "  := (fun (n0 : Free Shape Pos (Integer Shape Pos)) =>"
        ++ "        xs >>= (fun (xs_0 : List Shape Pos a) =>"
        ++ "        @foo_0 Shape Pos a n0 xs_0))" ++ "     (succ Shape Pos n)."
  it "translates polymorphically recursive functions correctly"
    $ shouldSucceedWith $ do
      ...
      shouldConvertWithHelpersTo
        ...
        ++ " := match t with" ++ "     | leaf x => pure 1%Z"
        ++ "     | fork t' => succ Shape Pos" ++ "         (t' >>="
  ...
  it ("translates recursive functions with strict and non-strict"
      ++ "arguments correctly") $ shouldSucceedWith $ do
    "List" <- defineTestTypeCon "List" 1 ["Nil", "Cons"]
    ("nil", "Nil") <- defineTestCon "Nil" 0 "forall a. List a"
    ("cons", "Cons") <- defineTestCon "Cons" 2 "forall a. a -> List a -> List a"
    ...
    shouldConvertWithHelpersTo
      [ "interleave @a (xs :: List a) !(ys :: List a) :: List a ="
          ++ "  case xs of {" ++ "    Cons x xs' ->" ++ "      case ys of {"
          ++ "        Cons y ys' ->"
      ...
      ++ "  {struct xs}" ++ " := match xs with" ++ "    | cons x xs' =>"
      ++ "        match ys with" ++ "        | cons y ys' =>"
      ++ "            @Cons Shape Pos a x"
      ++ "              (@Cons Shape Pos a y"
      ++ "                (xs' >>= (fun (xs'_0 : List Shape Pos a) =>"
      ++ "                  ys' >>= (fun (ys'_0 : List Shape Pos a) =>"
      ++ "                    @interleave_0 Shape Pos a xs'_0 ys'_0))))"
      ++ "        | nil => pure xs" ++ "        end" ++ "    | nil => pure ys"
      ++ "    end. "
  ...
  it ("converts recursive functions with a strict argument preceding"
      ++ "the decreasing argument correctly") $ shouldSucceedWith $ do
    "List" <- defineTestTypeCon "List" 1 ["Nil", "Cons"]
    ("nil", "Nil") <- defineTestCon "Nil" 0 "forall a. List a"
    ("cons", "Cons") <- defineTestCon "Cons" 2 "forall a. a -> List a -> List a"
    "foo"
    ...
    shouldConvertWithHelpersTo
      [ "foo @a !(x :: a) (xs :: List a) :: a =" ++ "  case xs of {"
          ++ "    Cons x' xs' -> foo @a x xs';" ++ "    Nil         -> x"
          ++ "  }"
      ] $ "(* Helper functions for foo *) "
      ++ "Fixpoint foo_0 (Shape : Type) (Pos : Shape -> Type) {a : Type}"
      ++ "  (x : a) (xs : List Shape Pos a) {struct xs}" ++ " := match xs with"
  ...
  it ("translates recursive functions affected by the eta conversion"
      ++ "pass correctly") $ shouldSucceedWith $ avoidLaziness $ do
    "List" <- defineTestTypeCon "List" 1 ["Nil", "Cons"]
    ("nil", "Nil") <- defineTestCon "Nil" 0 "forall a. List a"
    ("cons", "Cons") <- defineTestCon "Cons" 2 "forall a. a -> List a -> List a"
    ...
    "append"
      <- defineTestFunc "append" 3 "forall a b. List a -> List a -> b -> List a"
    shouldConvertWithHelpersTo
      [ "append @a @b (xs :: List a) (ys :: List a) :: b -> List a ="
          ++ "  \\y -> const @(List a) @b" ++ "    (case xs of {"
          ++ "      Nil      -> ys;"
          ++ "      Cons x xs' -> Cons @a x (append @a @Unit xs' ys Tt)"
          ++ "    } :: List a) y"
      ]
      ("(* Helper functions for append *)" ++ " Fixpoint append_0"
       ++ "   (Shape : Type) (Pos : Shape -> Type) {a b : Type}"
       ++ "   (xs : List Shape Pos a)"
       ++ "   (ys : Free Shape Pos (List Shape Pos a))"
       ++ "   {struct xs} : Free Shape Pos (List Shape Pos a)"
       ++ "  := match xs with" ++ "     | nil => ys" ++ "     | cons x xs' =>"
       ++ "         @Cons Shape Pos a x" ++ "           ((fun y =>"
