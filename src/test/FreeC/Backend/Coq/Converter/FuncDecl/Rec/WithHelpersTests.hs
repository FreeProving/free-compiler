...
testConvertRecFuncDeclWithHelpers = context "with helper functions"
  $ do
    ...
    it "translates mutually recursive functions correctly"
      ...
      $ do
        ...
        shouldConvertWithHelpersTo
          ...
          ++ "Fixpoint even_len_0 (Shape : Type) (Pos : Shape -> Type)"
    ...
    it "translates recursive functions with nested case expressions correctly"
      ...
      $ do
        ...
        ("cons", "Cons")
          <- defineTestCon "Cons" 2 "forall a. a -> List a -> List a"
        "tails"
          <- defineTestFunc "tails" 1 "forall a. List a -> (List (List a))"
        shouldConvertWithHelpersTo
    ...
    it ("converts recursive functions with a strict argument preceding"
      ...
      $ do
        ...
        ("cons", "Cons")
          <- defineTestCon "Cons" 2 "forall a. a -> List a -> List a"
        "foo" <- defineStrictTestFunc "foo" [True, False]
          "forall a. a -> List a -> a"
        shouldConvertWithHelpersTo
