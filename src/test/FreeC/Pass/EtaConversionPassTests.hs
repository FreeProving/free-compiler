...
testEtaConversionPass = describe "FreeC.Pass.EtaConversionPass"
  $ do
...
testEtaConvertFuncDecl = context "etaConvertFuncDecl"
  $ do
    it ("applies functions under-applied on the top-level"
    ...
    it "updates function arity in environment"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 0 []
        _ <- defineTestFunc "f" 0 "Foo -> Foo"
    ...
    it "updates function return type in environment"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 0 []
        _ <- defineTestFunc "f" 0 "Foo -> Foo"
    ...
    it "updates function argument type in environment"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 0 []
        _ <- defineTestFunc "f" 0 "Foo -> Foo"
    ...
    it ("applies under-applied function that is if-expression"
    ...
    it ("applies under-applied function that is if-expression to minimal"
    ...
    it "works with mutually recursive functions"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 0 []
        ...
        _ <- defineTestFunc "f" 0 "Foo -> Foo"
        _ <- defineTestFunc "g" 2 "Foo -> Foo -> Foo"
        input <- parseTestModule [ "module Test where"
                                 , "k :: Foo -> Foo = f;"
                                 , "f :: Foo -> Foo = g (k Foo)"
                                 ]
...
testEtaConvertExpr = context "etaConvertExpr"
  $ do
    it "leaves fully applied functions unchanged"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 0 []
        _ <- defineTestFunc "f" 2 "Foo -> Foo -> Foo"
        "f x y" `shouldEtaConvert` "f x y"
    it "leaves over applied functions unchanged"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 0 []
        _ <- defineTestFunc "f" 2 "Foo -> Foo -> Foo"
        "f x y z" `shouldEtaConvert` "f x y z"
    it "eta-converts under applied functions"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 0 []
        _ <- defineTestFunc "f" 2 "Foo -> Foo -> Foo"
    ...
    it "leaves application of local variables unchanged"
      ...
      $ do
        shouldEtaConvert "\\(f :: a -> b -> c) x -> f x"
          "\\(f :: a -> b -> c) x -> f x"
    it "leaves fully applied constructors unchanged"
      $ shouldSucceedWith
      $ do
        _ <- defineTestTypeCon "Foo" 0 []
