...
generateInductionSchemes dataDecls = do
  ...
  -- Generate names.
  forQualidMap <- Map.fromList
    <$> mapM (generateName "For" "" . IR.typeDeclQName) complexDataDecls
  inQualidMap <- Map.fromList
    <$> mapM (generateInNames . IR.typeDeclQName) complexDataDecls
  schemeQualidMap <- Map.fromList
    <$> mapM (generateName "" "_ind" . IR.typeDeclQName) dataDecls
  forallQualidMap <- Map.fromList
    <$> mapM (generateName "For" "_forall" . IR.typeDeclQName) complexDataDecls
  -- Generate properties.
  forBodies <- mapM (generateForProperty forQualidMap) complexDataDecls
  (inBodies', inConNames) <- mapAndUnzipM (generateInProperties inQualidMap)
    complexDataDecls
  ...
  forallBodies <- zipWithM (generateForallLemma schemeQualidMap forallQualidMap
                            forQualidMap inQualidMap) inConNames
    complexDataDecls
  hintSentences <- concatMapM
    (generateHints schemeQualidMap forallQualidMap forQualidMap inQualidMap)
    complexDataDecls
  -- Insert names into environment.
  mapM_ (insertPropertiesInEnv forQualidMap inQualidMap forallQualidMap
         . IR.typeDeclQName) complexDataDecls
  -- Return result
  return
    (Coq.commentedSentences
     ("ForType properties for " ++ showPretty (map IR.typeDeclName dataDecls))
     [Coq.InductiveSentence (Coq.Inductive (NonEmpty.fromList forBodies) [])
     | not (null forBodies)
     ]
     ++ Coq.commentedSentences
     ("InType properties for " ++ showPretty (map IR.typeDeclName dataDecls))
     [Coq.InductiveSentence (Coq.Inductive (NonEmpty.fromList inBodies) [])
     | not (null inBodies)
     ]
     ++ Coq.commentedSentences
     ("Induction schemes for " ++ showPretty (map IR.typeDeclName dataDecls))
     (map (\(name, binders, term, proof) -> Coq.AssertionSentence
           (Coq.Assertion Coq.Lemma name binders term) proof) schemeBodies)
     ++ Coq.commentedSentences
     ("Forall lemmas for " ++ showPretty (map IR.typeDeclName dataDecls))
     (map (\(name, binders, term, proof) -> Coq.AssertionSentence
           (Coq.Assertion Coq.Lemma name binders term) proof) forallBodies)
     ++ Coq.commentedSentences "Give hints" hintSentences)
 where
  -----------------------------------------------------------------------------
  -- @ForType@ Properties                                                    --
  -----------------------------------------------------------------------------
  ...
  generateForProperty
    :: LookupMap Coq.Qualid -> IR.TypeDecl -> Converter Coq.IndBody
  generateForProperty _ (IR.TypeSynDecl _ _ _ _)
    = error "generateForProperty: Type synonym not allowed"
  generateForProperty forQualidMap
    (IR.DataDecl _ (IR.DeclIdent srcSpan typeName) typeVarDecls conDecls) = do
      -- Generate constructor names.
      forConQualids <- mapM (generateConName forQualid . IR.conDeclQName)
        conDecls
      -- Enter local environment.
      localEnv $ do
        -- Collect and generate relevant Coq names.
        typeQualid <- lookupIdentOrFail srcSpan IR.TypeScope typeName
        (typeVarQualids, typeVarBinders)
        ...
        forCons <- zipWithM
          (generateForConstructor typeVarQualids propertyQualids) conDecls
          forConQualids
        -- Stick everything together.
        let propertyTypes   = map
              (\a -> Coq.Arrow (Coq.Qualid a) (Coq.Sort Coq.Prop))
              typeVarQualids
            propertyBinders = zipWith
              (Coq.typedBinder' Coq.Ungeneralizable Coq.Explicit)
              propertyQualids propertyTypes
            binders         = genericArgDecls Coq.Explicit
              ++ typeVarBinders
              ++ propertyBinders
            returnType      = Coq.Arrow
              (genericApply typeQualid [] [] (map Coq.Qualid typeVarQualids))
              (Coq.Sort Coq.Prop)
   ...
   where
    ...
    generateForConstructor typeVarQualids propertyQualids
      (IR.ConDecl _ (IR.DeclIdent srcSpan' conName) args) forConQualid
      = localEnv $ do
        -- Collect and generate relevant Coq names.
        conQualid <- lookupIdentOrFail srcSpan' IR.ValueScope conName
        (argQualids, binders) <- unzip
          <$> mapM (convertAnonymousArg . Just) args
        -- Generate a hypothesis for every argument of the constructor.
        -- But ignore trivial hypotheses.
        forHypotheses
          <- catMaybes <$> zipWithM generateForHypothesis argQualids args
        -- Generate constructor.
        let forResult  = genericApply forQualid [] []
              (map Coq.Qualid typeVarQualids
               ++ map Coq.Qualid propertyQualids
               ++ [ genericApply conQualid [] (map Coq.Qualid typeVarQualids)
                      (map Coq.Qualid argQualids)
                  ])
            returnType = Coq.forall binders
              (foldr Coq.Arrow forResult forHypotheses)
     ...
     where
      ...
      generateForHypothesis
        :: Coq.Qualid -> IR.Type -> Converter (Maybe Coq.Term)
      generateForHypothesis argQualid argType = do
        ...
        return $ case mbHyp of
          Just hyp -> Just
            ...
            [coqType, hyp, Coq.Qualid argQualid]
          Nothing  -> Nothing
      ...
      generateForHypothesis_2 d (IR.TypeCon _ tconName) typeArgs     = do
        ...
        if (tconArity == length typeArgs) && not (null $ catMaybes hypotheses)
          then do
            ...
            mbForType <- if tconName == typeName
              && all (\(tvar, targ) -> Coq.Qualid tvar == targ)
              (zip typeVarQualids coqArgs)
              && d <= maxDepth
              then 
                -- Legal recursion. 
            ...
            return ((\forType -> genericApply forType [] []
                     (coqArgs ++ hypotheses')) <$> mbForType)
          else return Nothing
  ...
  generateInNames typeName = do
    ...
    inQualids <- map snd
      <$> if arity == 1
        ...
        else mapM (\index -> generateName "In" ("_" ++ show index) typeName)
          [1 .. arity]
    return (typeName, inQualids)

  -- | Generates the 'In-' properties for a given data declaration.
  generateInProperties :: LookupMap [Coq.Qualid]
                       -> IR.TypeDecl
                       -> Converter ([Coq.IndBody], [Coq.Qualid])
  generateInProperties _ (IR.TypeSynDecl _ _ _ _)
    = error "generateInProperty: Type synonym not allowed"
  generateInProperties inQualidMap
    (IR.DataDecl _ (IR.DeclIdent _ typeName) typeVarDecls conDecls) = do
      (bodies, inConNames) <- mapAndUnzipM
        (generateInProperty inQualidMap typeName typeVarDecls conDecls)
        [0 .. length typeVarDecls - 1]
  ...
  --   in @x@.
  generateInProperty :: LookupMap [Coq.Qualid]
                     -> IR.QName
                     -> [IR.TypeVarDecl]
                     -> [IR.ConDecl]
                     -> Int
  ...
  generateInProperty inQualidMap typeName typeVarDecls conDecls index = do
    ...
    (cons, mkBody) <- localEnv $ do
      ...
      let binders    = genericArgDecls Coq.Explicit ++ typeVarBinders
          returnType = Coq.Arrow (Coq.Qualid $ typeVarQualids !! index)
            (Coq.Arrow (genericApply typeQualid [] []
                        (map Coq.Qualid typeVarQualids)) (Coq.Sort Coq.Prop))
    ...
    let cons' = zipWith (\inConName mbConType -> (inConName, [], mbConType))
          inConNames
          $ map snd cons
   ...
   where
    ...
    generateInConstructors
      :: [Coq.Qualid] -> IR.ConDecl -> Converter [(IR.QName, Maybe Coq.Term)]
    generateInConstructors typeVarQualids
      (IR.ConDecl _ (IR.DeclIdent srcSpan conName) args) = localEnv $ do
        -- Collect and generate relevant Coq names.
        conQualid <- lookupIdentOrFail srcSpan IR.ValueScope conName
        (argQualids, argBinders) <- unzip
          <$> mapM (convertAnonymousArg . Just) args
        elemQualid <- freshCoqQualid "x"
        -- Find occurrences of the relevant type variable in the arguments.
        occurrences <- concatMapM (uncurry $ findOccurrences elemQualid)
          $ zip argQualids args
        ...
        let inResult = genericApply inQualid [] []
            ...
            elemBinder = Coq.typedBinder' Coq.Ungeneralizable Coq.Explicit
              elemQualid elemType
     ...
     where
      ...
      inHypothesis
        :: Coq.Qualid -> [Coq.Term] -> Coq.Qualid -> Coq.Qualid -> Coq.Term
      inHypothesis inQualid' typeArgs containerQualid elemQualid
        = genericApply inQualid' [] []
        (typeArgs ++ [Coq.Qualid elemQualid, Coq.Qualid containerQualid])

      -- | Find occurrences of the relevant type variable in the given type.
      findOccurrences :: Coq.Qualid
                      -> Coq.Qualid
                      -> IR.Type
                      -> Converter [([Coq.Binder], [Coq.Term])]
      findOccurrences elemQualid argQualid argType = do
        ...
        coqType <- convertType' argType
        argType' <- expandAllTypeSynonyms argType
        findOccurrences_1 0 elemQualid
          (inHypothesis Coq.Base.inFree [coqType] argQualid) argType'

      -- | Find occurrences of the relevant type variable in the given type.
      --   Memorizes the depth of the current search path.
      findOccurrences_1 :: Int
                        -> Coq.Qualid
                        -> (Coq.Qualid -> Coq.Term)
                        -> IR.Type
                        -> Converter [([Coq.Binder], [Coq.Term])]
      ...
      findOccurrences_1 d elemQualid mkInHyp fullType@(IR.TypeApp _ _ _) =
       ...
       where
        -- | Unfolds a type application.
        findOccurrences_2
          :: IR.Type -> [IR.Type] -> Converter [([Coq.Binder], [Coq.Term])]
        ...
        findOccurrences_2 (IR.TypeCon _ tconName) typeArgs     = localEnv $ do
          ...
          Just tconArity <- inEnv $ lookupArity IR.TypeScope tconName
          if tconArity == length typeArgs then do
            coqArgs <- mapM convertType' typeArgs
            -- Prevent mutual recursion in the hypotheses and prevent
            -- direct recursion which is deeper than @maxDepth@.
            mbInTypes <- if tconName == typeName
              && all (\(tvar, targ) -> Coq.Qualid tvar == targ)
              (zip typeVarQualids coqArgs)
              && d <= maxDepth
              then 
            ...
            case mbInTypes of
              Just inTypes -> do
                ...
                (containerQualid, containerBinder) <- convertAnonymousArg'
                  (Just fullType)
                occurrences <- concatMapM
                  (\(it, arg) -> findOccurrences_1 (d + 1) elemQualid
                   (inHypothesis it coqArgs containerQualid) arg)
                  $ zip inTypes typeArgs
                let mkNewOcc (occBinders, inHypotheses)
                      = ( containerBinder : reverse occBinders
                        , mkInHyp containerQualid : inHypotheses
                        )
                return $ map mkNewOcc occurrences
              Nothing      -> return [] else return []
  ...
  generateSchemeLemma schemeQualidMap forQualidMap
    (IR.DataDecl _ (IR.DeclIdent srcSpan typeName) typeVarDecls conDecls)
    = localEnv $ do
      -- Collect and generate relevant Coq names.
      typeQualid <- lookupIdentOrFail srcSpan IR.TypeScope typeName
   ...
   where
    ...
    generateInductionCase propQualid
      (IR.ConDecl _ (IR.DeclIdent srcSpan' conName) argTypes) = localEnv $ do
        -- Collect and generate relevant Coq names.
        conQualid <- lookupIdentOrFail srcSpan' IR.ValueScope conName
  ...
  generateForallLemma
    ...
    -> IR.TypeDecl
    -> Converter (Coq.Qualid, [Coq.Binder], Coq.Term, Coq.Proof)
  generateForallLemma _ _ _ _ _ (IR.TypeSynDecl _ _ _ _)
    = error "generateForallLemma: Type synonym not allowed"
  generateForallLemma schemeQualidMap forallQualidMap forQualidMap inQualidMap
    inConNames (IR.DataDecl _ (IR.DeclIdent _ typeName) typeVarDecls _)
    = localEnv $ do
      Just typeQualid <- inEnv $ lookupIdent IR.TypeScope typeName
      (tvarQualids, tvarBinders)
        <- convertTypeVarDecls' Coq.Explicit typeVarDecls
      (propQualids, propBinders) <- mapAndUnzipM
        (\tv -> generateArg "P" (Coq.Arrow (Coq.Qualid tv) (Coq.Sort Coq.Prop)))
        tvarQualids
      (valQualid, valBinder) <- generateArg freshArgPrefix
        (genericApply typeQualid [] [] (map Coq.Qualid tvarQualids))
      inTerms <- zipWithM (generateInTerm valQualid tvarQualids) [0 ..]
        propQualids
      let forallQualid    = forallQualidMap Map.! typeName
          ...
          binders         = genericArgDecls Coq.Explicit
            ...
            ++ propBinders
            ++ [valBinder]
          lhs             = genericApply forQualid [] []
            (map Coq.Qualid $ tvarQualids ++ propQualids ++ [valQualid])
          rhs             = let (inQualids', [lastIn])
                                  = splitAt (length inTerms - 1) $ inTerms
                            in foldr Coq.conj lastIn inQualids'
          term            = Coq.forall binders (Coq.equiv lhs rhs)
          vars            = map (fromJust . Coq.unpackQualid)
            (Coq.Base.shape : Coq.Base.pos : tvarQualids ++ propQualids)
   ...
   where
    ...
    generateInTerm
      :: Coq.Qualid -> [Coq.Qualid] -> Int -> Coq.Qualid -> Converter Coq.Term
    generateInTerm valQualid tvarQualids index propQualid = localEnv $ do
      let inQualid = (inQualidMap Map.! typeName) !! index
      (val2Qualid, val2Binder) <- generateArg "y"
        (Coq.Qualid $ tvarQualids !! index)
      let isIn = genericApply inQualid [] []
            (map Coq.Qualid $ tvarQualids ++ [val2Qualid, valQualid])
      return
        $ Coq.forall [val2Binder]
        $ Coq.Arrow isIn
        (Coq.app (Coq.Qualid propQualid) [Coq.Qualid val2Qualid])
    ...
    generateForallHint inCon
      = let Just inStr = Coq.unpackQualid inCon
        in "  Local Hint Extern 1 => "
           ++ Text.unpack Coq.Base.proveForallFinish
           ++ ' '
           : inStr ++ " : " ++ Text.unpack Coq.Base.proveForallDb ++ ".\n"
  ...
  --   'forall' sentences.
  generateHints :: LookupMap Coq.Qualid
                -> LookupMap Coq.Qualid
                -> LookupMap Coq.Qualid
                -> LookupMap [Coq.Qualid]
                -> IR.TypeDecl
                -> Converter [Coq.Sentence]
  generateHints _ _ _ _ (IR.TypeSynDecl _ _ _ _)
    = error "generateHint: Type synonym not allowed"
  generateHints schemeQualidMap forallQualidMap forQualidMap inQualidMap
    (IR.DataDecl _ (IR.DeclIdent _ typeName) typeVarDecls tconDecls) = do
      ...
      proveIndHint <- generateProveIndHint typeName forType forall scheme
        (length typeVarDecls) tconDecls
      proveForallHint1 <- generateProveForallHint1 forType forall
        (length typeVarDecls)
      proveForallHints2 <- mapM
        (generateProveForallHint2 forType forall (length typeVarDecls)) inTypes
  ...
  --   @Coq.Base.proveIndProveForType@.
  generateProveIndHint
    :: IR.QName
    -> Coq.Qualid
    -> Coq.Qualid
    -> Coq.Qualid
  ...
  generateProveIndHint typeName forType forall scheme nTvars conDecls = do
    ...
    let tacticConStr   = Text.unpack Coq.Base.proveIndProveForType
        ...
        tactic         = unwords [tacticConStr, valStr, forallStr, schemeStr]
          ++ (if null unfoldSubProps
                then ""
                else ";\nrepeat (\n" ++ tacticUnlines unfoldSubProps ++ ")")
    return
      $ Coq.externHint (Just Coq.Global) 0 (Just forTypePattern) tactic
      [Coq.Base.proveIndDb]
   where
    ...
    unfoldSubProp' dName inT = localEnv $ do
      ...
      let forStr     = unpackQualid' forT
          ...
          forPatStrs = forStr
            : replicate (2 + 2 * dArity) "_" ++ ['?' : valStr2]
          inPatStrs  = inStr
            : replicate (2 + dArity) "_" ++ ['?' : valStr1, '?' : valStr2]
          tactic     = unlines'
            [ "  try match goal with"
            ...
            , "        |- _ ] =>"
            , "          "
                ++ unwords [ Text.unpack Coq.Base.proveIndForTypeInType
                           , hForStr
                           , hInStr
                           , valStr1
                           , forallStr
  ...
  -- | Generates a hint for forall lemma generation, using the template
  --   @Coq.Base.proveForallProveForType@.
  generateProveForallHint1
    :: Coq.Qualid -> Coq.Qualid -> Int -> Converter Coq.Sentence
  generateProveForallHint1 forType forall nTvars = do
    let tacticConStr   = Text.unpack Coq.Base.proveForallProveForType
        Just forallStr = Coq.unpackQualid forall
    ...
    return
      $ Coq.externHint (Just Coq.Global) 0 (Just forTypePattern) tactic
      [Coq.Base.proveForallDb]

  -- | Generates a hint for forall lemma generation, using the template
  --   @Coq.Base.proveForallForTypeInType@.
  generateProveForallHint2
    :: Coq.Qualid -> Coq.Qualid -> Int -> Coq.Qualid -> Converter Coq.Sentence
  generateProveForallHint2 forType forall nTvars inType = localEnv $ do
    hForStr <- freshCoqIdent "HF"
    hInStr <- freshCoqIdent "HI"
    valStr1 <- freshCoqIdent freshArgPrefix
    valStr2 <- freshCoqIdent freshArgPrefix
    let tacticConStr   = Text.unpack Coq.Base.proveForallForTypeInType
        Just forStr    = Coq.unpackQualid forType
        Just inStr     = Coq.unpackQualid inType
        Just forallStr = Coq.unpackQualid forall
        forPatStrs     = forStr
          : replicate (2 + 2 * nTvars) "_" ++ ['?' : valStr2]
        inPatStrs      = inStr
          : replicate (2 + nTvars) "_" ++ ['?' : valStr1, '?' : valStr2]
        tactic         = unlines
          [ ""
          , "  match goal with"
          , "  | [ " ++ hForStr ++ " : " ++ unwords forPatStrs
    ...
    return
      $ Coq.externHint (Just Coq.Global) 0 Nothing tactic
      [Coq.Base.proveForallDb]
  ...
  -- | Checks whether the type eclaration includes at least one type variable.
  hasTypeVar :: IR.TypeDecl -> Bool
  hasTypeVar (IR.TypeSynDecl _ _ _ _)
    = error "hasTypeVar: Type synonym not allowed"
  hasTypeVar (IR.DataDecl _ _ typeVarDecls _) = not $ null typeVarDecls

  -- | Generate a name from a name by adding a prefix and a suffix.
  generateName
  ...
  -- | Generate a name from a name by adding the constructor name to it.
  ...
  -- | Returns the name corresponding 'For-' property by using the given Map
  --   first and the environment on failure.
  getForType
    :: LookupMap Coq.Qualid -> IR.QName -> Converter (Maybe Coq.Qualid)
  ...
  -- | Insert the property names and the forall lemma for a type in the
  --   environment.
  insertPropertiesInEnv :: LookupMap Coq.Qualid
                        -> LookupMap [Coq.Qualid]
                        -> LookupMap Coq.Qualid
                        -> IR.QName
                        -> Converter ()
  ...
  -- | Generates an argument with the given name and type.
