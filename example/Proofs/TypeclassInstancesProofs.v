...
Example nondeterministic_list : (@handle _ _ _ (HandlerND _)) ndList
...
(* (foo baz) ? (foo (bar (foo baz)))
   --> [ foo baz, foo (bar (foo baz)) ] *)
Example nondeterministic_foo : (@handle _ _ _ (HandlerND _)) ndFoo
...
Example nondeterministic_tree : (@handle _ _ _ (HandlerND _)) ndTree
...
Example nondeterministic_map : (@handle _ _ _ (HandlerND _)) ndMap
...
: (@handle _ _ _ (HandlerShareND _)) (doubleDisjunctionHead _ _ Cbneed (ND.Partial _ _) ndList2)
= [true;false].
Proof. trivial. Qed.
...
: (@handle _ _ _ (HandlerShareND _)) (doubleDisjunctionRoot _ _ Cbneed (ND.Partial _ _) ndTree)
= [true;false].
Proof. trivial. Qed.
...
: (@handle _ _ _ (HandlerShareND _)) (doubleDisjunctionHeadRoot _ _ Cbneed (ND.Partial _ _) ndTree2)
= [true;false].
Proof. trivial. Qed.
...
: (@handle _ _ _ (HandlerShareND _)) (doubleDisjunctionMap _ _ Cbneed (ND.Partial _ _) ndMap)
= [true;false].
Proof. trivial. Qed.
