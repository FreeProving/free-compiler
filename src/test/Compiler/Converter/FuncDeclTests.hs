...
testConvertRecFuncDeclsWithHelpers =
  describe "Compiler.Converter.FuncDecl.convertRecFuncDeclsWithHelpers" $ do
    ...
    it "translates partial recursive functions correctly"
      ...
      ++ "           if cond_0"
      ++ "           then x_1"
      ++ "           else xs'_1 >>= (fun (xs'_2 : List Shape Pos a) =>"
      ++ "                      findJust_1 P xs'_2))"
      ++ "     end. "
    ...
    it "translates recursive functions with outer lambda abstractions correctly"
      ...
      ++ "Fixpoint append_1 (xs : List Shape Pos a) {struct xs}"
      ++ " := match xs with"
      ++ "    | nil            => ys_0"
      ++ "    | cons x_1 xs'_1 => Cons Shape Pos x_1 (xs'_1 >>="
      ++ "      (fun (xs'_2 : List Shape Pos a) =>"
      ++ "          append_1 xs'_2))"
      ++ "    end. "
      ++ "(* Main functions for append *) "
      ++ "Definition append_0 (xs : Free Shape Pos (List Shape Pos a))"
      ++ " : Free Shape Pos (List Shape Pos a)"
      ++ " := xs >>= (fun (xs_0 : List Shape Pos a) =>"
      ++ "   append_1 xs_0). "
