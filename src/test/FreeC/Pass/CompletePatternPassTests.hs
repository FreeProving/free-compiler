...
testCompletePatternPass = describe "FreeC.Pass.CompletePatternPass"
  $ do
    context "Top-level case expressions"
      $ do
        it "fails when a constructor is missing"
          $ do
            input <- expectParseTestFuncDecl
              "f x = case x :: Foobar of {Foo -> Foo}"
            shouldFail
              $ do
        ...
        it "fails when a constructor occurs more than once"
          $ do
            input <- expectParseTestFuncDecl
            ...
            shouldFail
              $ do
                ...
                _ <- defineTestVar "x"
                checkPatternFuncDecl input
        it "succeeds when all constructors occur exactly once"
          $ do
            input <- expectParseTestFuncDecl
          ...
          $ do
            ...
            shouldFail
              $ do
                ...
                _ <- defineTestVar "x"
                checkPatternFuncDecl input
    context "Nested and deeper case expressions"
      $ do
        it "fails for a faulty case expression inside an if statement"
          $ do
            input <- expectParseTestFuncDecl
            ...
            shouldFail
              $ do
                ...
                _ <- defineTestVar "b"
                checkPatternFuncDecl input
        it "fails for a faulty nested case expression"
          $ do
            input <- expectParseTestFuncDecl
          ...
          $ do
            ...
            shouldFail
              $ do
                ...
                _ <- defineTestVar "x"
                checkPatternFuncDecl input
        it "succeeds for a valid nested case expression"
          $ do
            input <- expectParseTestFuncDecl
              $ "f x = case x :: Foobar of {Foo -> case x :: Foobar of "
            ...
            shouldSucceed
              $ do
                ...
                _ <- defineTestVar "x"
                checkPatternFuncDecl input
        it "fails for a faulty case expression inside a lambda expression"
          $ do
            input <- expectParseTestFuncDecl
            ...
            shouldFail
              $ do
                ...
                _ <- defineTestVar "x"
                checkPatternFuncDecl input
    context "Illegal scrutinee types"
      $ do
          ...
          $ do
            ...
            shouldSucceed
              $ do
                ...
                _ <- defineTestVar "x"
                checkPatternFuncDecl input
        it "fails if the case expression's scrutinee is a lambda expression"
          $ do
            input <- expectParseTestFuncDecl
            ...
            shouldFail
              $ do
                ...
                _ <- defineTestCon "Bar" 0 "Foobar"
                checkPatternFuncDecl input
        it "fails if the case expression's scrutinee is polymorphic"
          $ do
    ...
    context "Type synonyms"
      $ do
        it "succeeds for complete pattern matching on a type synonym"
          $ do
            input <- expectParseTestFuncDecl
            ...
            shouldSucceed
              $ do
                ...
                checkPatternFuncDecl input
        it "succeeds for complete pattern matching on a nested type synonym"
          $ do
            input <- expectParseTestFuncDecl
