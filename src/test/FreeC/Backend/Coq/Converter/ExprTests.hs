...
testConvertFuncApp = context "function applications" $ do
  ...
  it "converts complete function applications correctly" $ shouldSucceedWith
    $ do
  ...
  it "converts unapplied functions correctly" $ shouldSucceedWith $ do
    ...
    "a" <- defineTestTypeVar "a"
    "f @a" `shouldConvertExprTo` "@f Shape Pos a"
  it "converts applications of partial functions correctly" $ shouldSucceedWith
    $ do
  ...
  it "converts function applications with two strict arguments correctly"
    $ shouldSucceedWith $ do
      ...
      "x" <- defineTestVar "x"
      "y" <- defineTestVar "y"
      shouldConvertExprTo "f @a x y" $ "x >>= (fun (x_0 : a) =>"
...
testConvertCase = context "case expressions" $ do
  ...
  it "uses data (not smart) constructors" $ shouldSucceedWith $ do
    ...
    shouldConvertExprTo "case e of { C1 -> e1;  C2 -> e2 }"
      $ "e >>= (fun e_0 =>" ++ "  match e_0 with" ++ "  | c1 => e1"
      ++ "  | c2 => e2" ++ "  end)"
  it "allows case expressions to shadow local variables" $ shouldSucceedWith
    $ do
      ...
      shouldConvertExprTo "case e of { Nil -> x; Cons x xs -> x }"
        $ "e >>= (fun e_0 =>" ++ "  match e_0 with" ++ "  | nil => x"
        ++ "  | cons x0 xs => x0" ++ "  end)"
  it "allows case expressions to shadow local variables" $ shouldSucceedWith
    $ do
      ...
      shouldConvertExprTo
        ...
        $ "x >>= (fun x_1 =>" ++ "  match x_1 with"
        ++ "  | a => x >>= (fun x_2 =>" ++ "    match x_2 with"
        ++ "    | a => x" ++ "    | b => x" ++ "    end)" ++ "  | b => x"
        ++ "  end)"
...
testConvertLambda = context "lambda abstractions" $ do
  ...
  it "allows lambda abstractions to shadow local variables" $ shouldSucceedWith
    $ do
      "x" <- defineTestVar "x"
...
testConvertInteger = context "integer expressions" $ do
  it "translates zero correctly" $ shouldSucceedWith
    $ shouldConvertExprTo "0" "pure 0%Z"
  it "translates positive decimal integer literals correctly"
    $ shouldSucceedWith $ shouldConvertExprTo "42" "pure 42%Z"
  it "translates hexadecimal integer literals correctly" $ shouldSucceedWith
    $ shouldConvertExprTo "0xA2" "pure 162%Z"
  it "translates octal integer literals correctly" $ shouldSucceedWith
