module FreeC.Analysis.TypeInferenceTests where
...
testInferFuncDeclTypes =
  describe "FreeC.Analysis.TypeInference.inferFuncDeclTypes" $ do
...
testInferExprType = describe "FreeC.Analysis.TypeInference.inferExprType" $ do
  ...
  it "infers the type of partially applied predefined operations correctly"
    ...
    $ do
        shouldInferType
          "(+) 42"
          ("\\x@0 -> Prelude.(+) 42 x@0", "Prelude.Integer -> Prelude.Integer")
  ...
  it "expands type synonyms when necessary" $ shouldSucceed $ fromConverter $ do
    "Foo"  <- defineTestTypeSyn "Foo" [] "[Integer]"
