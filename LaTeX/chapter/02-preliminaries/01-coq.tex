...
For example, the following \coq{Definition} sentence declares a function \coq{isEmpty} that tests whether a list \coq{xs} is empty or not.
\begin{minted}{coq}
...
\end{minted}
On the right-hand side of \coq{isEmpty} we are using a \coq{match} expression to perform pattern matching.
...
One crucial difference between functions in Haskell and Coq is, that Coq distinguishes between non-recursive functions~--~like \coq{isEmpty}~--~and recursive functions.
...
For example, the following sentence declares a theorem called \coq{isEmpty_length} that states that if a list is empty, its length is zero.
\begin{minted}{coq}
...
Of course, the proof scripts of \coq{isEmpty_length} and other real theorems and the proofs constructed by them are much more complicated than that.
What may be confusing is that the proposition on the right-hand side of \coq{isEmpty_length} does not even look like a type anymore since it contains values like \coq{true} and \coq{0}.
