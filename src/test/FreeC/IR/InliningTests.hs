...
testInlining :: Spec
testInlining = describe "FreeC.IR.Inlining"
  $ do
    context "inlineExpr"
      $ do
        ...
        it "inlines transitive"
          ...
          $ do
            ...
            return (res `shouldBeSimilarTo` expected)
        it "inlines in a function application"
          $ shouldSucceedWith
          $ do
            _ <- defineTestTypeCon "Integer" 0 []
            _ <- defineTestFunc "h" 1 "forall a. a -> a"
            ...
            return (res `shouldBeSimilarTo` expected)
        it "inlines in an if expression"
          $ shouldSucceedWith
          $ do
            ...
            return (res `shouldBeSimilarTo` expected)
    context "inlineFuncDecls"
      $ do
        it "inlines a recursive function correctly"
          $ shouldSucceedWith
          $ do
            ...
            expected <- parseTestFuncDecl
              ("add (m :: Peano) (n :: Peano) :: Peano = case m of "
               ++ "{ Zero -> n ; Succ m' -> Succ (add m n) }")
            return (res `shouldBeSimilarTo` expected)
        it "inlines the zigzag example correctly"
          $ shouldSucceedWith
          $ do
            _ <- defineTestTypeCon "Bool" 0 ["True", "False"]
